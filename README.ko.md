...
![Fog Harbor 모험 시작 화면](./docs/assets/readme/ttrpg-current.png)
...
![StoryForge 제품 목록](./docs/assets/readme/home-current.png)

| 목적 | 진입점 | 결과와 현재 상태 |
|---|---|---|
| 장편 소설 쓰기 | 장편 소설 | 인물, 구성, 장, 연속성, 개정. 현재 기술적 제작 흐름 검증 완료 |
| 짧은 작품 완성하기 | 단편 소설 | 설계, 장 카드, 집필, 검토, 불변 판본. Markdown/TXT/JSON |
| 소설을 시나리오로 각색 | 소설에서 시나리오 | 고정 원전, 각색 판단, 구조화 장면, 검토. Fountain/FDX/인쇄 |
| 소설을 만화로 각색 | 소설에서 만화 | 페이지, 컷, 시각 참조, 레터링, 판본. PNG/WebP/CBZ/PDF |
| 세계 만들기 | 세계 엔진 | 의미 콘텐츠, 명시적 파생, 불변 버전, 읽기 |
...
1. **장편**(`长篇`) → **작품 라이브러리**(`作品库`) → **새 장편**(`新建长篇`)에서 제목을 입력합니다。
...
![직접 작성한 예시 구성을 사용하는 장편 작업 공간](./docs/assets/readme/longform-outline-current.png)
