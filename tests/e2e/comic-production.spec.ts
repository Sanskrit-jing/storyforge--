...
test('小说转漫画从独立产品入口冻结来源，经十二步正式数据发布不可变分镜版', async ({ page }) => {
  ...
  const showcase = page.getByTestId('comic-showcase')
  await expect(showcase).toBeVisible()
  await expect(showcase.getByRole('heading', { name: '连续成页，才是真正的小说转漫画' })).toBeVisible()
  await showcase.getByRole('button', { name: '查看完整漫画《雨停之前》' }).click()
  await expect(page.getByRole('dialog', { name: '《雨停之前》' })).toBeVisible()
  await page.getByRole('button', { name: '关闭漫画预览' }).click()
