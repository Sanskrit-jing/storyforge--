...
const mode = process.env.PRODUCTPROD_PERF_MODE === 'commercial' ? 'commercial' : 'smoke'
const commercialQuality = mode === 'commercial' || process.env.PRODUCTPROD_PERF_QUALITY === 'commercial'
const commercialDurationMs = 30 * 60 * 1000
const smokeDurationMs = 5_000
const configuredWarmupTransitions = Number(process.env.PRODUCTPROD_PERF_WARMUP_TRANSITIONS ?? 20)
...
function productionOutputs(system: string) {
  ...
  if (system.includes('任务=content.design。')) return {
    schema: 'storyforge.product-design-artifact', version: 1, title: '性能验收循环',
  ...
  if (system.includes('任务=content.narrative。')) return {
    schema: 'storyforge.product-narrative-artifact', version: 1, moduleKind: 'main', moduleTitle: '潮门性能路线',
  ...
  if (system.includes('任务=content.product-module。')) return {
    schema: 'storyforge.product-module-artifact', version: 1, productType: 'avg',
...
async function createPerformanceBuild(page: Page) {
  ...
  if (commercialQuality) {
    await expect(page.getByTestId('product-production-performance-blocker')).toContainText('尚无浏览器性能回执')
    await expect(page.getByTestId('product-production-playthrough-blocker')).toContainText('试玩未发布 Build')
  ...
  await expect(page.getByTestId('avg-player')).toBeVisible({ timeout: 15_000 })
  await expect(page.getByTestId('avg-player')).toContainText('潮门信号第一次亮起。')
}

async function revealChoices(page: Page) {
  const choicesVisible = () => page.evaluate(() => Array.from(document.querySelectorAll<HTMLElement>('.avg-choices'))
    .some(element => {
      const rect = element.getBoundingClientRect()
      const style = getComputedStyle(element)
      return rect.width > 0 && rect.height > 0 && style.visibility !== 'hidden' && style.display !== 'none'
    }))
  for (let attempt = 0; attempt < 4 && !await choicesVisible(); attempt += 1) {
    const control = page.locator('button.avg-continue:not([disabled])').last()
    // A durable beat commit briefly disables the control and may replace it
    // directly with the choice panel. Wait for either valid next state instead
    // of treating that expected DOM replacement as a click failure.
    await expect.poll(async () => (
      await choicesVisible() || await control.isVisible().catch(() => false)
    ), { timeout: 15_000 }).toBe(true)
    if (await choicesVisible()) break
    try {
      await control.click({ timeout: 5_000 })
    } catch (cause) {
      if (!await choicesVisible()) throw cause
    }
  }
  await expect.poll(choicesVisible, { timeout: 15_000 }).toBe(true)
}
...
async function measureChoiceTransition(page: Page) {
  ...
  const choice = page.locator('.avg-choices button:not(:disabled)').first()
  ...
  await expect(page.locator('.avg-continue')).toBeVisible({ timeout: 10_000 })
...
async function recordReceipt(page: Page, measurement: BrowserMeasurement) {
  return page.evaluate(async input => {
    const importer = new Function('path', 'return import(path)') as (path: string) => Promise<any>
    const { recordProductBrowserPerformanceMeasurementV1 } = await importer('/storyforge/src/lib/product-production/quality-receipts.ts')
    const { db } = await importer('/storyforge/src/lib/db/schema.ts')
    const build = await db.productBuilds.orderBy('id').last()
    if (!build) throw new Error('性能验收 Build 不存在')
...
test('真实浏览器采样写入 Build 回执；smoke 不冒充商业通过', async ({ page, browserName }, testInfo) => {
  ...
  if (commercialQuality) {
    await revealChoices(page)
    await page.locator('.avg-choices button').filter({ hasText: '公开记录' }).click()
    await expect(page.locator('.avg-ending h2')).toHaveText('公开记录')
  }

  const recorded = await recordReceipt(page, measurement)
  await attachReceipt(testInfo, recorded)
  console.log(`[PRODUCTPROD-PERF] ${JSON.stringify({
  ...
  if (commercialQuality) {
    const playthroughBlocker = page.getByTestId('product-production-playthrough-blocker')
    ...
    const playthroughReceipt = page.getByTestId('product-production-playthrough-receipt')
    await expect(playthroughReceipt).toContainText('结局 truth-ending')
    await expect(playthroughReceipt).toContainText(/· \d+ 次选择/)
    await expect(page.getByTestId('product-production-playthrough-blocker')).toHaveCount(0)
  }
  if (mode === 'commercial') {
    await expect(page.getByText('#1 · 可发布', { exact: true })).toBeVisible()
    await expect(page.getByTestId('product-production-performance-blocker')).toHaveCount(0)
    ...
    await expect(page.getByTestId('product-production-performance-blocker')).toContainText('long-run-incomplete')
    await expect(page.getByRole('button', { name: '复验并原子发布', exact: true })).toHaveCount(0)
  }
})
