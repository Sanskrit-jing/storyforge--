...
test('漫剧素材独立入口、无作品浏览与内容分类',async({page})=>{
 ...
 for(const label of ['作品库','小说来源','系列设定','物料库','单集内容','工具适配包','质量审查','版本与交付','通用设置'])await expect(nav.getByRole('button',{name:label,exact:true})).toBeVisible()
 await nav.getByRole('button',{name:'单集内容',exact:true}).click()
...
test('一句话仍先建立小说来源，不冒充已完成素材',async({page})=>{
 ...
 await page.getByRole('navigation',{name:'漫剧素材页面导航'}).getByRole('button',{name:'系列设定',exact:true}).click()
