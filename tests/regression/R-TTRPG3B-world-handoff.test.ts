...
describe('TTRPG-3B · WorldRelease to TTRPG production handoff', () => {
  ...
  it('世界引擎正式按钮只携带跑团身份与冻结 release 进入产品制作页', () => {
    ...
    expect(productHub).toContain('const parsed = parseProductProductionHandoffV1(JSON.parse(encoded))')
    ...
    expect(productHub).toContain('release.contentHash !== parsed.worldContentHash')
