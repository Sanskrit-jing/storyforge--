...
describe('COMIC-2 · professional novel-to-comic pipeline', () => {
  ...
  it('十二步正式数据闭合，分镜/视觉双层发布冻结 Blob，并完成 v14 往返', async () => {
    ...
    const normalizedRequest = await generateComicProfessionalCandidateV1({ scope: item.scope, adaptationProjectId: root.id!, stage: 'image-request', targetPanelKeys: [plannedPanel.stableKey], runAI: async () => JSON.stringify({ panelKey: 'panel_wrong', expectedPanelRevision: 999, visualPrompt: '林岚站在旧车站，no text, no speech balloons, no watermark', negativePrompt: 'no text, no speech balloons, no watermark', referenceSubjectKeys: ['hero'], protectedAreas: [{ x: 0, y: 0, width: 1, height: 1 }] }) }); expect((normalizedRequest.candidate.payload as any).panelKey).toBe(plannedPanel.stableKey); expect((normalizedRequest.candidate.payload as any).protectedAreas).toEqual([]); await rejectComicProfessionalCandidateV1(item.scope, normalizedRequest.snapshot.run.id)
    let pageReviewRepairCalls = 0; const repairedReview = await generateComicProfessionalCandidateV1({ scope: item.scope, adaptationProjectId: root.id!, stage: 'page-review', targetPageKeys: [plannedPage.stableKey], runAI: async () => { pageReviewRepairCalls += 1; return pageReviewRepairCalls === 1 ? JSON.stringify([{ stableKey: 'issue_wrong_panel', category: 'narrative', severity: 'major', pageKey: 'page_wrong', panelKey: 'panel_wrong', subjectKey: null, assetKey: null, evidence: '目标键错误。', problem: '面板定位错误。', suggestion: '改用目标格。', sourceUnitKeys: [item.unit.sourceUnitKey] }]) : '[]' } }); expect(pageReviewRepairCalls).toBe(2); await adoptComicProfessionalCandidateV1({ scope: item.scope, runId: repairedReview.snapshot.run.id })
    ...
    let visualReviewCalls = 0; const visualReview = await generateComicProfessionalCandidateV1({ scope: item.scope, adaptationProjectId: root.id!, stage: 'visual-continuity-review', targetPageKeys: [page.stableKey], runAI: async messages => { visualReviewCalls += 1; if (visualReviewCalls === 1) { expect(messages[0].content).toContain(`assetKey 闭集：${asset.stableKey}`); expect(messages[1].content).toContain('不传图片像素'); return JSON.stringify([{ ...visualIssue, assetKey: 'asset_outside_current_adaptation' }]) } return JSON.stringify([visualIssue]) } }); expect(visualReviewCalls).toBe(2)
