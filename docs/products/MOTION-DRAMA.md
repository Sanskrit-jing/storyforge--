...
> 版本：1.2.0 · 生效：2026-09-15 · 权威层级：L2
> 产品：`independent.motion-drama` · 用户名称：漫剧素材
> 实施状态：已完成当前工程闭环；能力边界与证据以能力基线和自动验证为准。

## 1. 产品目标

用户入口统一称“漫剧素材”，稳定产品 ID 和已有数据不改名。默认面向 Seedance，交付到可供外部工具使用的逐镜提示词与参考素材包；“保存交付版”只冻结这些前期内容，不代表生成、剪辑或发布漫剧成片。已有其他 provider 适配保持兼容。

漫剧素材把一句话或已有小说生产为可交给外部 AI 图片/视频工具的专业前期生产包：小说来源、系列策划、漫剧剧本、版本化物料、逐镜分镜、分镜图/关键帧提示词、参考帧和视频生成提示词适配包。
