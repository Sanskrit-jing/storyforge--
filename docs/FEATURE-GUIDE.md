...
- AI 会优先尊重已写明的东西南北、远近、里程和城池/国家规模；没有明确距离时会标记
  比例尺为“系统估算”。
- 比例尺可手动调整并按当前世界保存；若资料彼此矛盾，地图信息栏会显示空间关系冲突，
  不会静默丢弃设定。
- 多世界模式下可按世界分别维护。
...
- 生成后可把选中的卷或章节导入大纲。
- 已有正文时可切换到“中途重规划”，说明新增角色或弧光变化。系统会按真实写作进度保护已写章，并给出轻量、中度、深度三档未来大纲方案。
- 中途重规划不会自动修改正文或故事主线；作者需逐项查看标题/摘要变化并确认，应用前系统还会检查大纲是否已经被手工改动。

适合用法：开书时用角色弧光搭建大纲；连载中途人物发生变化时，用重规划先看影响，再只调整尚未写作的部分。
...
写入正文后，续写、整理本章、影响分析和质量审校等按钮会从禁用变为可用。正文编辑器提供字体、字号、行距、段距、字色、背景色、加粗、斜体、标题、列表、引用、分割线、撤销和重做等富文本工具。

“整理本章”只发送一次正文，集中生成角色状态、受控事实、物品流水、故事年表、角色关系和伏笔推进候选。所有条目都会展示正文逐字证据；你可以逐项取消，确认前不会修改项目。候选保存在本地，刷新后仍可继续审核；正文变化后旧候选会被标为过期并禁止写回。

正文保存后，一致性 Agent 会自动运行零 token 的确定性 Fast Guard；只有发现当前正文存在
高置信物品连续性问题时，“质量审校”才显示当前告警数。需要更完整的检查时，打开
“质量审校 → 一致性”，由作者明确选择 Fast Guard 或 Deep Review：Fast 最多读取 16K
登记上下文，Deep 最多读取 32K，两者每次最多调用模型一次。报告会显示当前/过期状态、
调用次数、预算与实际来源，刷新后仍可恢复；报告只读，不会自动改正文或任何项目数据。
...
- 用“整理本章”统一审核状态、事实、物品、年表、关系和伏笔候选。
- 确认后的事实仍只进入事实候选区，需要在事实库再次确认才会成为 Canon。
- 做影响分析，检查历史章修改对后续事实的影响。
- 打开质量审校，检查因果、动机、伏笔和一致性问题；一致性页提供零 token 后台守卫和
  作者显式运行的 Fast/Deep 报告。
...
工作区侧栏“体验中心 → 互动运行时”是跑团、NPC 演进和角色聊天后续共用的独立存档入口。
它不会把游戏中的时间、骰子或叙事自动写回小说正文、角色主档和正式 Canon。

当前可用功能：

- 创建沙盒、NPC 演进、跑团或角色聊天类型的本地会话。
- 推进整数逻辑时间，使用 `NdM±K` 骰式进行可复现判定。
- 追加只属于当前会话的叙事，查看继承后的当前叙事状态和本会话事件日志。
- 建立带完整状态与校验 hash 的命名检查点。
- 从当前事件位置建立独立分支；父会话后续变化不会改写子分支。
- 刷新后恢复会话、事件、检查点和分支；删除会话前需二次确认。
- 会话随项目 JSON 导出导入，并在删除项目或所属世界时统一清理。

当前边界：会话类型只是共享运行时的可见壳，跑团主持和角色聊天产品尚未完成；现阶段也
不会自动读取项目角色与世界设定。下一阶段会让作者明确选择要冻结进会话的 Canon 来源，
并把来源、字段和 hash 展示出来。
...
- 章节里的“整理本章”经作者勾选确认后才会更新角色状态卡与其它章后产物。
