...
- `src/lib/simulation/runtime.ts` 以纯 reducer 回放严格连续事件；时间、实体、记忆、叙事和
  ...
  不能提交自选点数。
- 工作区“体验中心 → 互动运行时”已提供沙盒、NPC 演进、跑团和角色聊天四类会话壳；
...
- 后台领域任务目前按依赖顺序执行，不是并行自治团队；当前有三档领域输入预算、一次
  受控确定性打回、手动“整理本章”和只读一致性 Agent。任意单源权重、更多 Canon 闭集、
  模型投票和 NPC 演进仍未形成正式产品闭环。
- SIM-1A 当前新建会话只保存空来源快照说明；尚未交付作者选择式世界/角色/地点/物品/
  规则冻结、运行时实体投影、检查点恢复或 AI 动作候选。四类会话类型是共享引擎入口，
  不代表 TTRPG 或角色聊天产品已经完成。
