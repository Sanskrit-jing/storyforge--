...
| 10 | **SIM-1 世界模拟与互动运行时** | 为跑团、角色聊天、文字游戏和 NPC 演进提供共同状态与事件地基 | SIM-1A 已交付三表、事件回放、确定性骰子、检查点/分支、便携生命周期和体验中心可见会话壳；下一步 SIM-1B 交付作者选择式 Canon 冻结快照与运行时实体投影 | 创作 Canon 与运行时状态分层；模拟不得污染作者原稿；状态变化可回放、可分支 | **SIM-1A COMPLETE；SIM-1B IN PROGRESS（2026-07-27）** |
...
| 13 | **PRODUCT-1 新手转化、数据主权与开源信任** | 让新用户快速得到成果，成熟用户敢托付手稿，贡献者能参与 | `AUDIT-5/8/9/10/11`、升级前快照、加密备份、帮助系统、i18n、安全/贡献/发布政策 | 首次成果闭环、Labs 隐藏、备份恢复可信、隐私和贡献流程清楚 | **DESIGN；可错峰** |
| 14 | **PLATFORM-1 协作与社区广场** | 支撑作品发布、发现、讨论、协作和社区治理 | 协同编辑、账号/权限、云同步、发布发现、评论社群、内容治理 | 先完成后端、身份、同步冲突、隐私和治理架构；不在纯前端上硬叠 | **LONG-TERM；独立架构立项** |
| 15 | **KB-1 全局知识库** | 让作者手写的跨项目查阅式参考手册，供 AI 写到相关情节时按主题主动查询取用 | 已完成：DB v49 表 + `PROJECT_TABLES` 登记 + 条目结构（主题标题+可选触发词+范例正文）+ `search_knowledge` 读取工具（AI 主动查询）+ `knowledgeSelection`（作者显式勾选）与 `knowledgeQuery`（查询上下文）两源 + 首页独立知识库页（/knowledge）管理与 JSON v2 导出导入 | 不常驻注入：默认装配不含知识内容；匹配引擎按标题/触发词/正文计分排序；反例测试覆盖不注入、工具参数校验与生命周期 | **COMPLETE（2026-09-22）** |
...
12. ✅ SIM-1A 共同运行时核心与可见会话壳（2026-07-27）：DB v48 三表、严格追加事件、
    ...
    刷新恢复和安全删除独立会话。
13. ⏳ SIM-1B Canon 冻结选择与运行时实体投影 → NPC 演进 → TTRPG-1 /
    CHATGAME-1 → PRODUCT-1 / PLATFORM-1：互动产品必须先共享 SIM 状态层，不为 NPC、
    跑团和角色聊天各建一套引擎。
14. ✅ KB-1 全局知识库（2026-09-22）：作者手写跨项目知识条目为查阅式参考手册，
    不常驻注入任何创作装配；AI 经 `search_knowledge` 工具按主题主动查询
    （标题/触发词/正文计分匹配），节点模式可显式勾选条目（`knowledgeSelection`）
    或由查询生成 `knowledgeQuery` 上下文；注册表、生命周期与首页独立页管理已收口，
    反例测试覆盖不注入、工具参数校验与 JSON v2 往返。

### FLOW-1 历史实验 / FLOW-2 当前交付卡

- **历史结论**：FLOW-1 的 `PromptWorkflow.graph` 只证明了 DAG 校验和画布交互可行，
  不是独立节点产品；它继续留在提示词管理中服务分步骤配方。
- **当前入口**：工作区“节点模式”加载独立 `NodeModeWorkspace`；作者可创建项目级节点图，
  添加六类节点、自由拖动/连线、动态增加输入槽并运行全图或任意祖先闭包。
...
- **当前非范围**：条件、循环、并行、脚本/插件、图模板、脏下游自动失效和 Agent 自动生成图。
