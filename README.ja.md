...
![Fog Harbor のアドベンチャー開始画面](./docs/assets/readme/ttrpg-current.png)
...
![StoryForge の製品一覧](./docs/assets/readme/home-current.png)

| 目的 | 入口 | 得られるもの・現在の状態 |
|---|---|---|
| 長編小説を書く | 長編小説 | 人物、構成、章、連続性、改稿。現行の技術的な制作フローは検証済み |
| 短い作品を完成させる | 短編小説 | 設計、章カード、執筆、レビュー、固定版の公開。Markdown/TXT/JSON |
| 小説を脚本にする | 小説から脚本 | 固定した原典、改編判断、構造化シーン、レビュー。Fountain/FDX/印刷 |
| 小説を漫画にする | 小説から漫画 | ページ、コマ、画像参照、文字組み、公開版。PNG/WebP/CBZ/PDF |
| 世界を作る | 世界エンジン | 意味内容、明示的な派生、固定バージョン、読み取り |
...
1. **長編**（`长篇`）→ **作品ライブラリ**（`作品库`）→ **新しい長編**（`新建长篇`）でタイトルを入力します。
...
![手書きのデモ構成を使った長編ワークスペース](./docs/assets/readme/longform-outline-current.png)
