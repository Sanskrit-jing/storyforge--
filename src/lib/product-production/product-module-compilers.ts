...
import { parseOpenWorldEvolutionContent } from '../open-world/evolution-runtime'
...
export function compileOpenWorldEvolutionModuleV1(input: ProductModuleCompilerInputV1): OpenWorldEvolutionContentV1 {
  ...
  const issues: OpenWorldEvolutionContentV1['issues'] = sourceNodes.map((node, index) => {
  ...
  const actors: OpenWorldEvolutionContentV1['actors'] = [
  ...
  const actions: OpenWorldEvolutionContentV1['actions'] = input.narrative.choices.map((choice, index) => {
  ...
  return parseOpenWorldEvolutionContent({
...
export function compileOpenWorldModulesV1(input: ProductModuleCompilerInputV1) {
  ...
  const openWorldEvolution = compileOpenWorldEvolutionModuleV1(input)
  ...
  const explicitOrganizations = openWorldEvolution.actors.filter(actor => actor.kind === 'organization').map(actor => actor.key)
  const organizations = explicitOrganizations.length ? explicitOrganizations : openWorldEvolution.actors.map(actor => actor.key)
  const regions: OpenWorldContentV1['regions'] = input.narrative.nodes.map((node, index) => {
    ...
    return {
      ...
      initialResources: { agency: openWorldEvolution.resources[0].initial },
      initialMetrics: Object.fromEntries(openWorldEvolution.metrics.map(metric => [metric.key, metric.initial])),
      initialIssuePressures: Object.fromEntries(openWorldEvolution.issues.map(issue => [issue.key, issue.initialPressure])),
  ...
  const actorSchedules: OpenWorldContentV1['actorSchedules'] = participantKeys.slice(0, 8).map((actorKey, index) => ({
    ...
    effects: [{
      ...
      key: `issue.${pad(index % openWorldEvolution.issues.length)}`, delta: 1,
    }],
  ...
  const openWorld = parseOpenWorldContent({
    ...
    evolutionCadenceTicks: 2, maxPropagationEdgesPerTick: 8,
    ...
    regionalIssueRules: openWorldEvolution.issues.map((issue, index) => ({
  ...
  return { interaction, adventure, openWorldEvolution, openWorld }
}
