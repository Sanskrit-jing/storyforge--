...
export async function readScreenplayAuthorDraft(scope: WorkspaceScope, key: string): Promise<string | null> {
  const conversation = await getOrCreateAgentConversation({ projectId:scope.projectId, scope, worldGroupId:null, purpose:`screenplay-author-draft:${key}`, title:'剧本编辑草稿' })
  const rows = await readAgentEvents(conversation.id!, scope)
  return rows.length ? rows[rows.length-1].content : null
}
export async function saveScreenplayAuthorDraft(scope: WorkspaceScope, key: string, content: string) {
  const conversation = await getOrCreateAgentConversation({ projectId:scope.projectId, scope, worldGroupId:null, purpose:`screenplay-author-draft:${key}`, title:'剧本编辑草稿' })
