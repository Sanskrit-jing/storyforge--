...
import { parseScreenplayModelJsonV1 } from './model-json'
import { screenplaySourceAnalysisUnitsV1 } from './source-analysis-units'
...
function repairGeneratedPortableKey(value: unknown, prefix: string): unknown {
  if (typeof value !== 'string' || /^[A-Za-z0-9][A-Za-z0-9._-]{0,79}$/.test(value)) return value
  if (!/^[A-Za-z0-9._\-\s:/]+$/.test(value)) return `${prefix}.gen.${stableHash(value)}`
  const repaired = value.trim().replace(/[\s:/]+/g, '-').replace(/^[._-]+|[._-]+$/g, '').slice(0, 80)
  return /^[A-Za-z0-9][A-Za-z0-9._-]{0,79}$/.test(repaired) ? repaired : `${prefix}.gen.${stableHash(value)}`
}

function portableSubjectKey(value: unknown): unknown {
  if (typeof value !== 'string' || /^[A-Za-z0-9][A-Za-z0-9._-]{0,79}$/.test(value)) return value
  return `subject.gen.${stableHash(value)}`
}

function namespaceGenericFactKey(row: Record<string, unknown>): void {
  if (typeof row.stableKey !== 'string' || (!/^fact[._-]?\d+$/i.test(row.stableKey) && !/^fact\.gen\.[a-f0-9]{8}$/i.test(row.stableKey)) || !Array.isArray(row.sourceUnitKeys) || typeof row.sourceUnitKeys[0] !== 'string') return
  const unitToken = row.sourceUnitKeys[0].replace(/^asu_/, '').replace(/[^A-Za-z0-9_-]/g, '').slice(0, 10)
  const suffix = row.stableKey.replace(/^fact[._-]?/i, '').replace(/^gen\./i, '')
  row.stableKey = `fact.${unitToken}.${suffix}`
}

/** Repairs only soft ASCII separators in model-created keys; reference keys remain fail-closed. */
export function normalizeScreenplayProviderKeysV1(stage: ScreenplayProfessionalStageV1, value: unknown): unknown {
  if (!value || typeof value !== 'object') return value
  if (Array.isArray(value)) {
    return value.map(item => {
      if (!item || typeof item !== 'object' || Array.isArray(item)) return item
      const row = { ...item } as Record<string, unknown>
      const prefix = stage === 'source-analysis' ? 'fact' : stage === 'causal-graph' ? 'edge' : stage === 'decision-pass' ? 'decision' : stage === 'beat-sheet' ? 'beat' : stage === 'scene-card' ? 'scene' : 'issue'
      if (['source-analysis', 'causal-graph', 'decision-pass', 'beat-sheet', 'scene-card', 'grounding-review', 'dramaturgy-review'].includes(stage)) row.stableKey = repairGeneratedPortableKey(row.stableKey, prefix)
      if (stage === 'source-analysis') {
        namespaceGenericFactKey(row)
        if (Array.isArray(row.subjectKeys)) row.subjectKeys = row.subjectKeys.map(portableSubjectKey)
      }
      if (stage === 'beat-sheet') {
        row.sectionKey = repairGeneratedPortableKey(row.sectionKey, 'section')
        if (!Object.prototype.hasOwnProperty.call(row, 'sectionTitle') && typeof row.sectionKey === 'string') row.sectionTitle = row.sectionKey
      }
      return row
    })
  }
  if (stage === 'scene-draft') {
    ...
    if (Array.isArray(row.blocks)) row.blocks = row.blocks.map(block => block && typeof block === 'object' && !Array.isArray(block) ? { ...block, id: repairGeneratedPortableKey((block as Record<string, unknown>).id, 'block') } : block)
    return row
  }
  return value
}
...
function exact(value: unknown, keys: readonly string[], label: string): asserts value is Record<string, any> {
  ...
  if (actual.length !== keys.length || actual.some(key => !keys.includes(key))) {
    ...
    throw new Error(`[screenplay-run] ${label} 字段不在闭集${unknown.length ? `；未知：${unknown.join('、')}` : ''}${missing.length ? `；缺少：${missing.join('、')}` : ''}`)
  }
}
...
function stableKey(value: unknown, label: string): asserts value is string {
  if (typeof value !== 'string' || !/^[A-Za-z0-9][A-Za-z0-9._-]{0,79}$/.test(value)) {
    const preview = typeof value === 'string' ? value.slice(0, 80) : typeof value
    throw new Error(`[screenplay-run] ${label} 非法：${preview}`)
  }
}
...
function portableKeyArray(value: unknown, label: string, allowEmpty = false): asserts value is string[] {
  stringArray(value, label, allowEmpty)
  if (value.some(item => !/^[A-Za-z0-9][A-Za-z0-9._-]{0,79}$/.test(item))) throw new Error(`[screenplay-run] ${label} 含非法稳定 key`)
}
...
function parseFact(value: unknown): asserts value is AdaptationSourceFactCandidateV1 {
  ...
  portableKeyArray(value.subjectKeys, 'SourceFact.subjectKeys', true); portableKeyArray(value.sourceUnitKeys, 'SourceFact.sourceUnitKeys')
...
function parseScene(value: unknown): asserts value is ScreenplaySceneCandidateV1 {
  ...
  const blockReport = validateScreenplayBlocksV1(value.blocks)
  if (!blockReport.valid) throw new Error(`[screenplay-run] SceneDraft blocks 非法：${blockReport.issues.filter(item => item.level === 'error').map(item => `${item.blockId ? `[${item.blockId}] ` : ''}${item.message}`).join('；')}`)
}
...
interface ScreenplayPromptReferenceKeysV1 {
  sourceUnitKeys: string[]
  factKeys: string[]
  decisionKeys: string[]
  beatKeys: string[]
  sceneCardKeys: string[]
  sceneCardPlanSectionKeys: Record<string, string>
}
...
function prompt(
  ...
  targetSpec: unknown,
  referenceKeys: ScreenplayPromptReferenceKeysV1,
): ChatMessage[] {
  ...
  const common = [
    ...
    '当输出是数组时，数组内每个 stableKey 必须唯一；同一候选中禁止复用 stableKey。场景 blocks 内每个 id 也必须唯一。',
    '证据不足时少写或输出空审查数组，不得虚构。先在内部静默核对字段闭集、来源引用和目标边界。',
    `目标剧本画像：${JSON.stringify(targetSpec)}`,
    `本次合法引用键清单（引用字段只能从这里逐字复制，禁止自造）：${JSON.stringify(referenceKeys)}`,
  ...
  const instructions: Record<ScreenplayProfessionalStageV1, string> = {
    'source-analysis': `按冻结来源提取 8～16 项高价值、可核对事实；合并纯气氛、重复动作和不影响因果或连续性的微小细节，不要逐句抄成事实清单。输出 JSON 数组；每项字段严格且仅为 ${FACT_KEYS.join(', ')}。每项形状必须是 {"stableKey":"fact.source-token.event","kind":"event","statement":"原文事实","subjectKeys":["character.zhaoning"],"sourceUnitKeys":["上下文给出的 sourceUnitKey"],"confidence":1}。stableKey 必须包含当前来源单元的 ASCII token 或具体语义，禁止使用 fact.001 一类跨单元会重复的纯编号；kind 只能是 event、character-state、relationship、location、object、motif 之一，禁止输出 action、character、plot、setting、theme 等近义词；confidence 必须是 0～1 的数字。statement 只陈述原文事实；subjectKeys 只能使用 ASCII 字母、数字、点、下划线或连字符组成的可移植语义键，例如 character.zhaoning，禁止 asu__character_赵宁 这类含中文的伪 key；不确定主体键时写 []。sourceUnitKeys 必须逐字使用上下文提供的 key 且至少一个。不得增加 evidence、quote、reasoning、category、id 等字段。`,
    ...
    'decision-pass': `对来源作 4～8 项高价值改编决定，不要求每条事实单独成项；输出必须以 [ 开始、以 ] 结束的紧凑 JSON 数组，不要前后文字。每项字段严格且仅为 ${DECISION_KEYS.join(', ')}，形状示例 {"stableKey":"decision.key","action":"keep","sourceFactKeys":["fact.key"],"targetKeys":[],"rationale":"不超过八十字的决定理由"}。action 只能是 keep、cut、merge、reorder、externalize、add 之一；sourceFactKeys 与 targetKeys 必须始终是无重复的字符串数组，除 add 外 sourceFactKeys 至少一个，add 才允许 []。不得增加 title、description、confidence、priority、id 等字段；内心活动优先 externalize 为行动、选择、对白、声音或已批准旁白。`,
    'beat-sheet': '输出 JSON Beat 数组。每项字段严格且仅为 stableKey, sectionKey, sectionTitle, scope, episodeNumber, order, objective, conflict, turn, outcome, causalFactKeys, decisionKeys, sourceUnitKeys, estimatedSeconds。形状示例 {"stableKey":"beat.opening","sectionKey":"act.one","sectionTitle":"第一幕","scope":"act","episodeNumber":1,"order":0,"objective":"目标","conflict":"冲突","turn":"转折","outcome":"结果","causalFactKeys":["fact.key"],"decisionKeys":["decision.key"],"sourceUnitKeys":["来源 key"],"estimatedSeconds":180}。scope 只能是 act、sequence、episode；episodeNumber/estimatedSeconds 必须是正整数；order 必须严格连续为 0、1、2……，不得跳号或重复。causalFactKeys 可空；每个 Beat 的 decisionKeys 与 sourceUnitKeys 都不可空，且只能逐字复制“本次合法引用键清单”中的 decisionKeys 与 sourceUnitKeys。电影 episodeNumber 固定 1，总 estimatedSeconds 必须落在目标分钟数乘 60 的正负 10% 内。约 20～30 分钟电影通常给出 8～12 个有因果推进的 Beat；更短或更长项目按时长调整密度。结构须形成清晰的开场压力、触发事件、升级、不可逆中点、危机、高潮和余波；每个 Beat 都写清角色当下目标、对抗、可见转折及改变后的结果，不能只写气氛或说明。sectionKey/sectionTitle 应组织成连贯的幕或 sequence。不得增加 act、title、description、duration、id 等字段。',
    ...
    'scene-draft': `只写目标 Scene Card 的一场 JSON AST 对象，顶层字段严格且仅为 ${SCENE_KEYS.join(', ')}。stableKey、episodeNumber、sceneNumber、estimatedSeconds、sourceUnitKeys 必须沿用 Card；planSectionKey 必须逐字复制“本次合法引用键清单”内 sceneCardPlanSectionKeys 为目标 Card 给出的值，绝不能写 beatKey；intExt 只能是 INT、EXT、INT_EXT。blocks 只能是数组：普通块严格为 {"id":"block.key","type":"action","text":"可见动作"}，type 只能为 action、parenthetical、dialogue、transition、shot、note；角色提示块为 {"id":"block.cue","type":"character","name":"角色名"}，可选 extension 只能为 V.O.、O.S.、O.C.、CONT'D，可选 dualDialogue 为布尔值。不得输出 characterId 或其他系统字段。每个 block.id 在本场必须唯一；每个 dialogue 前必须先有 character，且必须紧邻（中间只可有 parenthetical），任何广播、录音或画外对白也必须先写 character；parenthetical 只能位于 character 与 dialogue 之间，场末不能停在 character。角色 name 只写姓名或声源，不把（O.S.）等扩展重复写进 name，扩展只写入 extension。action 只写银幕上可见、可听、可执行的内容；事实说明要外化为画面、屏幕文本、道具、声音或对白，不写人物无法呈现的内心与小说式背景。动作段落短而有节奏，避免一段承载多个镜头；对白要有目的、对抗和潜台词，不复制来源长对白。`,
    'grounding-review': '只做来源忠实度与连续性审查。输出 JSON ReviewIssue 数组，每项字段严格且仅为 stableKey, category, severity, sceneKey, blockId, evidence, problem, suggestion, sourceUnitKeys。category 必须固定为 grounding；severity 只能是 critical、major、minor；sceneKey 必须等于目标场景 key；blockId 只能逐字复制目标场景已有且匹配 stable-key 正则的 block id，无法定位单块时必须写 JSON null，禁止空字符串、序号、描述或自造 id；sourceUnitKeys 必须是非空且只引用来源 key。不得增加 status、confidence、location、line、title、id 等字段。每项必须给出来源证据；合理且已确认的 add 不得误报，没有问题输出 []。',
    'dramaturgy-review': '只做戏剧审查：场景目的、冲突、动作化、节奏、人物声音与对白功能。输出 JSON ReviewIssue 数组，字段严格且仅为 stableKey, category, severity, sceneKey, blockId, evidence, problem, suggestion, sourceUnitKeys。category 必须固定为 dramaturgy；severity 只能是 critical、major、minor；sceneKey 必须等于目标场景 key；blockId 只能逐字复制目标场景已有且匹配 stable-key 正则的 block id，无法定位单块时必须写 JSON null，禁止空字符串、序号、描述或自造 id；sourceUnitKeys 必须是数组且允许 []。不得增加 status、confidence、location、line、title、id 等字段；evidence 引用目标场景文本，不臆造来源问题，没有问题输出 []。',
    'targeted-rewrite': '只修复作者选中的开放问题。输出单个 JSON 对象，字段严格且仅为 sceneKey, expectedSceneRevision, issueKeys, summary, blocks。expectedSceneRevision 必须是大于 0 的 JSON 整数，并精确复制登记上下文中目标场景的 current revision，不能使用字符串、null、0 或猜测值；sceneKey 必须等于目标场景 key，issueKeys 必须逐字复制本步全部目标问题 key。blocks 必须给出修订后的整场合法 AST，块形状和排列规则与 scene-draft 完全相同。保持场序、人物事实、来源、未授权台词与结构不变；不得输出 stableKey、status、revision、reasoning 或顺手重写别场。',
  }
...
async function selectedContext(input: { scope: WorkspaceScope; adaptationProjectId: number; stage: ScreenplayProfessionalStageV1; sourceUnitKeys?: string[]; targetSceneKeys?: string[]; targetIssueKeys?: string[] }) {
  ...
  const confirmedFactSourceKeys = input.sourceUnitKeys?.length || inferred.length || input.stage === 'source-analysis'
    ? []
    : [...new Set((await db.adaptationSourceFacts.where('[adaptationProjectId+manifestVersion]').equals([root.id!, root.activeSourceManifestVersion]).filter(row => row.authorStatus === 'confirmed').toArray()).flatMap(row => row.sourceUnitKeys))]
  const primarySourceKeys = screenplaySourceAnalysisUnitsV1(units).map(unit => unit.sourceUnitKey)
  const sourceUnitKeys = [...new Set(input.sourceUnitKeys?.length
    ? input.sourceUnitKeys
    : inferred.length
      ? inferred
      : confirmedFactSourceKeys.length
        ? confirmedFactSourceKeys
        : primarySourceKeys)]
...
function contract(scope: WorkspaceScope, stage: ScreenplayProfessionalStageV1) {
  ...
  return {
    ...
    budget: { maxModelCalls: 2, maxToolCalls: 0, maxInputTokens: 48_000, maxOutputTokens: skill.maxOutputTokens * 2, maxAttemptsPerStep: 2, maxProtocolErrors: 1 },
    acceptance: [
...
export async function generateScreenplayProfessionalCandidateV1(input: {
  ...
  assertPrerequisites(input.stage, selected.root, {
    ...
    scenes,
    issues,
  }, { scenes: input.stage === 'scene-draft' ? (input.targetSceneKeys?.length ?? 0) : selected.scenes.length, issues: selected.issues.length })
  ...
  const beatSectionByKey = new Map(beatRows.map(row => [row.stableKey, row.sectionKey]))
  const messages = prompt(input.stage, assembled.text, targetKeys, input.authorInstruction ?? '', selected.root.targetSpec, {
    sourceUnitKeys: [...selected.sourceUnitKeys],
    factKeys: factRows.map(row => row.stableKey).sort(),
    decisionKeys: decisionRows.map(row => row.stableKey).sort(),
    beatKeys: beatRows.map(row => row.stableKey).sort(),
    sceneCardKeys: cardRows.map(row => row.stableKey).sort(),
    sceneCardPlanSectionKeys: Object.fromEntries(cardRows.map(row => [row.stableKey, beatSectionByKey.get(row.beatKey) ?? ''])),
  })
  const runModel = async (attemptMessages: ChatMessage[], attempt: number) => {
  ...
  let finalAttempt = 1
  let raw = await runModel(messages, 1)
  let payload: ScreenplayProfessionalPayloadV1
  const parseAndCheckPayload = async (output: string) => {
    const parsed = parseScreenplayProfessionalPayloadV1(input.stage, normalizeScreenplayProviderKeysV1(input.stage, parseScreenplayModelJsonV1(output)))
    if (input.stage === 'scene-draft') await assertSceneCandidateMatchesCardV1(input.scope, parsed as ScreenplaySceneCandidateV1)
    return parsed
  }
  try { payload = await parseAndCheckPayload(raw) }
  catch (firstError) {
    const issue = firstError instanceof Error ? firstError.message : String(firstError)
    snapshot = await append(input.scope, snapshot, 'step.failed', { stepId: config.stepId, attempt: 1, code: `screenplay-${input.stage}-repairable-protocol`, retryable: true, category: 'protocol', action: 'retry' })
    snapshot = await append(input.scope, snapshot, 'step.started', { stepId: config.stepId, attempt: 2 })
    finalAttempt = 2
    finalMessages = [
      ...messages,
      {
        role: 'user',
        content: [
          '上一次输出未通过严格协议。只修复协议，不改变创作意图；重新输出完整且唯一的 JSON 值。',
          `校验错误：${issue.slice(0, 1_000)}`,
          `待修复输出：${raw.slice(0, 80_000)}`,
          input.stage === 'scene-draft'
            ? '逐块修复规则：删除没有对白的悬空 character；每个 dialogue 前补上正确角色的 character，或把本来就是动作描写的伪 dialogue 改为 action；character 与 dialogue 之间只能有 parenthetical；不得把角色姓名写进 dialogue 文本；重新检查完整 blocks 序列。'
            : '',
          '再次逐项核对字段闭集、合法引用键清单、非空数组、stable key 与数字类型。不要解释。',
        ].filter(Boolean).join('\n\n'),
      },
    ]
    ...
    try { payload = await parseAndCheckPayload(raw) }
      ...
      snapshot = await append(input.scope, snapshot, 'step.failed', { stepId: config.stepId, attempt: 2, code: `screenplay-${input.stage}-protocol-failed`, retryable: false, category: 'protocol', action: 'fail' })
      await append(input.scope, snapshot, 'run.failed', { code: `screenplay-${input.stage}-protocol-failed`, retryable: false })
      throw error
    }
  }
  const body = {
    ...
    contextManifestHash: contextManifest.manifestHash, promptHash: await hashCanonicalValue(finalMessages), modelOutputHash: await hashCanonicalValue({ raw }), payload, payloadHash: await hashCanonicalValue(payload),
  }
...
export async function adoptScreenplayProfessionalCandidateV1(input: {
  ...
  if (snapshot.projection.steps[config.stepId]?.status === 'running') snapshot = await append(input.scope, snapshot, 'step.succeeded', {
    stepId: config.stepId,
    attempt: snapshot.projection.steps[config.stepId]?.attempt ?? 1,
...
export async function readPendingScreenplayProfessionalCandidateV1(scope: WorkspaceScope): Promise<{ snapshot: AgentRunSnapshotV1; candidate: ScreenplayProfessionalCandidateV1 } | null> {
  ...
  for (const row of rows) {
    ...
    try {
      ...
      if (!snapshot.projection.steps[config.stepId]?.candidateHash && snapshot.projection.steps[config.stepId]?.status === 'running') snapshot = await append(scope, snapshot, 'candidate.persisted', {
        stepId: config.stepId,
        attempt: snapshot.projection.steps[config.stepId]?.attempt ?? 1,
