...
export async function adoptAdaptationSourceFactsV1(
  ...
  return db.transaction('rw', scopeTransactionTables(
    ...
    await db.adaptationProjects.update(root.id!, { ...(root.medium === 'screenplay' && root.plan ? { briefSourceManifestVersion: null, planSourceManifestVersion: null } : {}), revision: root.revision + 1, updatedAt: now })
