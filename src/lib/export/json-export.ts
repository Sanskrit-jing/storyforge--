...
/** 保存 JSON 文件（Web / 桌面触发下载，Android 走原生分享，见 save-file.ts） */
export async function downloadJSON(data: ProjectExportData, filename: string): Promise<void> {
  await saveText(JSON.stringify(data, null, 2), filename, 'application/json')
}
