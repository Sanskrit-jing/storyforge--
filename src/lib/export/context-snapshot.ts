...
/** 保存快照文件（Web / 桌面触发下载，Android 走原生分享，见 save-file.ts） */
export async function downloadContextSnapshot(text: string, projectName: string): Promise<void> {
  await saveText(
    text,
    `${projectName}_上下文快照_${new Date().toISOString().slice(0, 10)}.md`,
    'text/markdown',
  )
}
