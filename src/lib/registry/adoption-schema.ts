...
const ADOPTION_SCHEMAS_RAW: CollectionAdoptionSpec[] = [
  ...
  {
    target: 'comicScriptBeats',
    identity: 'id', recordOnly: true, duplicatePolicy: 'error', required: [],
    autoStamps: ['updatedAt'], ownerFrom: 'work',
  },
  {
    target: 'comicPagePlans',
    identity: 'id', recordOnly: true, duplicatePolicy: 'error', required: [],
    autoStamps: ['updatedAt'], ownerFrom: 'work',
  },
  {
    target: 'comicReviewIssues',
    identity: 'id', recordOnly: true, duplicatePolicy: 'error', required: [],
    autoStamps: ['updatedAt'], ownerFrom: 'work',
  },
  {
  ...
  {
    target: 'motionDramaSeriesBibles', identity: 'id', recordOnly: true,
    duplicatePolicy: 'error', required: [], autoStamps: [], ownerFrom: 'work',
  },
  {
    target: 'motionDramaEpisodes', identity: 'id', recordOnly: true,
    duplicatePolicy: 'error', required: [], autoStamps: ['updatedAt'], ownerFrom: 'work',
  },
  {
    target: 'motionDramaScriptScenes', identity: 'id', recordOnly: true,
    duplicatePolicy: 'error', required: [], autoStamps: ['updatedAt'], ownerFrom: 'work',
  },
  {
    target: 'motionDramaAssetSubjects', identity: 'id', recordOnly: true,
    duplicatePolicy: 'error', required: [], autoStamps: ['updatedAt'], ownerFrom: 'work',
  },
  {
    target: 'motionDramaShots', identity: 'id', recordOnly: true,
    duplicatePolicy: 'error', required: [], autoStamps: ['updatedAt'], ownerFrom: 'work',
  },
  {
    target: 'motionDramaReviewIssues', identity: 'id', recordOnly: true,
    duplicatePolicy: 'error', required: [], autoStamps: ['updatedAt'], ownerFrom: 'work',
  },
  {
    target: 'storyCores',
...
export const ADOPTION_EXTENSIONS: readonly AdoptionExtensionSpec[] = Object.freeze([
  ...
  {
    id: 'adaptation-source-fact-lifecycle',
    target: 'adaptationSourceFacts',
    entrypoints: ['src/lib/adaptation/analysis.ts'],
    policyRegistry: 'PROJECT_TABLES + FIELD_REGISTRY + closed source-fact contract + adaptation manifest CAS',
    reason: '来源事实候选经作者审定后按活动 manifest 原子替换；owner、稳定身份与作者状态不得由模型直接写入。',
    reviewAfter: '2027-09-01',
  },
  {
    id: 'adaptation-causal-edge-lifecycle',
    target: 'adaptationCausalEdges',
    entrypoints: ['src/lib/adaptation/analysis.ts'],
    policyRegistry: 'PROJECT_TABLES + FIELD_REGISTRY + closed causal-edge contract + same-manifest fact graph validator',
    reason: '因果边必须引用同一目标 Work、同一 manifest 的事实并拒绝自环，批次采纳必须原子完成。',
    reviewAfter: '2027-09-01',
  },
  {
    id: 'adaptation-decision-lifecycle',
    target: 'adaptationDecisions',
    entrypoints: ['src/lib/adaptation/analysis.ts'],
    policyRegistry: 'PROJECT_TABLES + FIELD_REGISTRY + closed adaptation-decision contract + adaptation manifest CAS',
    reason: '改编选择是作者审定的正式记录；除 add 外都必须追踪同一 manifest 的来源事实，候选不能旁路写库。',
    reviewAfter: '2027-09-01',
  },
  {
    id: 'screenplay-scene-lifecycle',
    target: 'screenplayScenes',
    entrypoints: ['src/lib/screenplay/service.ts', 'src/lib/screenplay/adoption.ts', 'src/lib/screenplay/production.ts'],
  ...
  {
    id: 'screenplay-beat-lifecycle',
    target: 'screenplayBeats',
    entrypoints: ['src/lib/screenplay/production.ts'],
    policyRegistry: 'PROJECT_TABLES + FIELD_REGISTRY + closed Beat contract + adaptation/source/decision CAS',
    reason: 'Beat 必须引用已确认来源事实和改编决定；重做时显式处理下游 Scene Card、场景与审查。',
    reviewAfter: '2027-09-01',
  },
  {
  ...
  },
  {
    ...
    target: 'comicPages',
    entrypoints: ['src/lib/comic/service.ts', 'src/lib/comic/production.ts'],
  ...
  {
    id: 'comic-script-beat-lifecycle',
    target: 'comicScriptBeats',
    entrypoints: ['src/lib/comic/production.ts'],
    policyRegistry: 'PROJECT_TABLES + FIELD_REGISTRY + closed comic beat contract + source/decision CAS',
    reason: '漫画脚本节拍必须追踪已确认事实、决定与来源，不能复用剧本 Beat 或直接从小说跳到页格。',
    reviewAfter: '2027-09-01',
  },
  {
    id: 'comic-page-plan-lifecycle',
    target: 'comicPagePlans',
    entrypoints: ['src/lib/comic/production.ts'],
    policyRegistry: 'PROJECT_TABLES + FIELD_REGISTRY + closed page rhythm contract + script beat CAS',
    reason: '页目标、翻页、格数与文字预算是漫画专用生产合同；重做时必须显式处理下游页格。',
    reviewAfter: '2027-09-01',
  },
  {
    id: 'comic-review-issue-lifecycle',
    target: 'comicReviewIssues',
    entrypoints: ['src/lib/comic/production.ts'],
    policyRegistry: 'PROJECT_TABLES + FIELD_REGISTRY + page/panel/subject/asset/revision validator',
    reason: '漫画审查问题必须定位当前页格和可见证据，修复只处理作者选择的问题。',
    reviewAfter: '2027-09-01',
  },
  {
    id: 'creation-release-asset-lifecycle',
    target: 'creationReleaseAssets',
    entrypoints: ['src/lib/comic/release.ts', 'src/lib/motion-drama/release.ts'],
    policyRegistry: 'PROJECT_TABLES + immutable release manifest + content hash + registered media strong reference',
    reason: 'visual Release 固定选片 Blob；草稿候选删除和媒体 GC 必须保留已发布页。',
    reviewAfter: '2027-09-01',
  },
  {
  ...
  {
    id: 'motion-drama-production-lifecycle',
    target: 'motionDramaProductions',
    entrypoints: ['src/lib/adaptation/source-manifest.ts', 'src/lib/motion-drama/service.ts', 'src/lib/motion-drama/prompts.ts', 'src/lib/motion-drama/prompt-pack.ts', 'src/lib/motion-drama/release.ts'],
    policyRegistry: 'PROJECT_TABLES + motion-drama state machine + adaptation source manifest CAS',
    reason: '漫剧阶段、当前集、系列圣经活动版本和发布指针必须与来源及逐集内容原子推进。',
    reviewAfter: '2027-09-09',
  },
  {
    id: 'motion-drama-series-bible-lifecycle',
    target: 'motionDramaSeriesBibles',
    entrypoints: ['src/lib/motion-drama/service.ts'],
    policyRegistry: 'PROJECT_TABLES + FIELD_REGISTRY + append-only bible version + adaptation CAS',
    reason: '系列圣经只接受作者确认候选并追加版本，生产根只移动活动版本指针。',
    reviewAfter: '2027-09-09',
  },
  {
    id: 'motion-drama-episode-lifecycle',
    target: 'motionDramaEpisodes',
    entrypoints: ['src/lib/motion-drama/service.ts'],
    policyRegistry: 'PROJECT_TABLES + FIELD_REGISTRY + episode contract + source manifest CAS',
    reason: '分集推进按单集覆盖并校验时长、节拍、来源和上下集连续性交接。',
    reviewAfter: '2027-09-09',
  },
  {
    id: 'motion-drama-script-scene-lifecycle',
    target: 'motionDramaScriptScenes',
    entrypoints: ['src/lib/motion-drama/service.ts'],
    policyRegistry: 'PROJECT_TABLES + FIELD_REGISTRY + scene/dialogue/sound closed contract + episode CAS',
    reason: '漫剧场景必须把可见动作、对白、旁白和声音 cue 分开并精确归属当前集。',
    reviewAfter: '2027-09-09',
  },
  {
    id: 'motion-drama-asset-subject-lifecycle',
    target: 'motionDramaAssetSubjects',
    entrypoints: ['src/lib/motion-drama/service.ts'],
    policyRegistry: 'PROJECT_TABLES + FIELD_REGISTRY + stable subject key + versioned media evidence',
    reason: '角色、服装、场景、道具和声音物料是系列级稳定锚点，提示词与真实参考素材不可混为一谈。',
    reviewAfter: '2027-09-09',
  },
  {
    id: 'motion-drama-shot-lifecycle',
    target: 'motionDramaShots',
    entrypoints: ['src/lib/motion-drama/service.ts'],
    policyRegistry: 'PROJECT_TABLES + FIELD_REGISTRY + shot duration/subject/source validation + dual prompt IR',
    reason: '镜头、静帧提示词和视频运动提示词分层确认，防止把多时刻动作塞进单一画面。',
    reviewAfter: '2027-09-09',
  },
  {
    id: 'motion-drama-review-issue-lifecycle',
    target: 'motionDramaReviewIssues',
    entrypoints: ['src/lib/motion-drama/service.ts'],
    policyRegistry: 'PROJECT_TABLES + FIELD_REGISTRY + current episode revision evidence',
    reason: '审查问题必须定位当前集的场景、镜头或物料，并区分内容、视听、权利与工具能力。',
    reviewAfter: '2027-09-09',
  },
  {
    id: 'motion-drama-media-lifecycle',
    target: 'motionDramaAssetVersions',
    entrypoints: ['src/lib/motion-drama/service.ts', 'src/lib/motion-drama/release.ts'],
    policyRegistry: 'PROJECT_TABLES + shared media blob + content hash + rights + explicit author selection',
    reason: '真实参考素材进入系统前必须校验 Blob、权利、来源和稳定版本，不能用文本提示词冒充已传参考图。',
    reviewAfter: '2027-09-09',
  },
  {
    id: 'motion-drama-prompt-pack-lifecycle',
    target: 'motionDramaPromptPacks',
    entrypoints: ['src/lib/motion-drama/service.ts', 'src/lib/motion-drama/prompts.ts', 'src/lib/motion-drama/prompt-pack.ts', 'src/lib/motion-drama/release.ts'],
    policyRegistry: 'provider-neutral Image/Video Prompt IR + deterministic provider adapters + content hash',
    reason: '工具包从已确认通用 IR 确定性编译，适配器不得反向改写剧情或媒资真相。',
    reviewAfter: '2027-09-09',
  },
  {
    id: 'motion-drama-asset-binding-lifecycle',
    target: 'motionDramaAssetBindings',
    entrypoints: ['src/lib/motion-drama/service.ts'],
    policyRegistry: 'PROJECT_TABLES + stable subject/version/shot keys + per-episode usage contract',
    reason: '物料绑定随来源显式重同步级联清理，并为后续逐镜绑定保留单一领域写入口。',
    reviewAfter: '2027-09-09',
  },
  {
    id: 'motion-drama-shot-reference-lifecycle',
    target: 'motionDramaShotReferences',
    entrypoints: ['src/lib/motion-drama/service.ts', 'src/lib/motion-drama/release.ts'],
    policyRegistry: 'PROJECT_TABLES + shared media blob + rights + one selected reference per shot role',
    reason: '镜头参考必须验证真实 Blob、授权和当前镜头归属，替换参考时显式取消旧选择并失效提示词包。',
    reviewAfter: '2027-09-09',
  },
  {
    id: 'motion-drama-prompt-override-lifecycle',
    target: 'motionDramaPromptOverrides',
    entrypoints: ['src/lib/motion-drama/prompts.ts'],
    policyRegistry: 'PROJECT_TABLES + work/episode override precedence + registered stage keys',
    reason: '用户提示词覆盖按作品或单集作用域保存，必须保持集级优先、作品级回退和内置默认三层确定性。',
    reviewAfter: '2027-09-09',
  },
  {
  ...
  {
    id: 'independent-creation-release-lifecycle',
    target: 'creationReleases',
    entrypoints: [
      'src/lib/short-novel/service.ts',
      'src/lib/screenplay/release.ts',
      'src/lib/comic/release.ts',
      'src/lib/motion-drama/release.ts',
    ],
    policyRegistry: 'SHORT_NOVEL_RELEASE_V1 + SCREENPLAY_RELEASE_V1 + COMIC_RELEASE_V1 + MOTION_DRAMA_RELEASE_V1 + PROJECT_TABLES + completion gates + immutable hash',
    reason: '独立短篇、剧本、漫画与漫剧前期包发布都必须从已审校 Canon 原子冻结，自包含且 append-only；所有格式导出只能读取已验证 manifest。',
    reviewAfter: '2027-09-06',
  },
  {
