 ...
 * 实现「连了文件夹就不怕重置」。
 *
 * Android 原生端没有 FSA,存的是 SAF 句柄(`{ saf, uri, name }`,纯数据)。
 * 两种句柄都可结构化克隆,故这里统一按 `FolderHandle` 存取,不做平台分派。
 */
import type { FolderHandle } from './folder-backup'
...
export async function saveFolderHandle(key: string, handle: FolderHandle): Promise<void> {
...
export async function loadFolderHandle(key: string): Promise<FolderHandle | null> {
  const db = await openDB()
  try {
    return await new Promise<FolderHandle | null>((resolve, reject) => {
      const tx = db.transaction(STORE, 'readonly')
      const r = tx.objectStore(STORE).get(key)
      r.onsuccess = () => resolve((r.result as FolderHandle) ?? null)
