...
export default function ProductFrame({ product, title, page, navigation, children }: {
  ...
  return <div className={`longform-app product-frame ${menu ? 'product-frame-menu-open' : ''}`}>
    <header className="lf-top"><Link className="lf-brand" to="/" aria-label="返回首页"><BrandIcon/><span><strong>StoryForge</strong><small>故事熔炉</small></span></Link><nav aria-label="产品导航">{PRODUCT_NAVIGATION.map(item => <Link key={item.id} to={item.path} aria-current={item.id === product ? 'page' : undefined}>{item.label}</Link>)}</nav></header>
