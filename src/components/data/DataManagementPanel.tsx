...
import {
  isFolderBackupSupported, isSafFolder, pickFolder, ensureFolderPermission,
  folderPermissionGranted, writeProjectJSONToFolder, type FolderHandle,
...
function ExportTab({ project, onImported }: Props) {
  ...
  const handleExportJSON = async () => {
    try {
      ...
      await downloadJSON(data, `${project.name}_${new Date().toISOString().slice(0, 10)}.json`)
  ...
  const handleExportMarkdown = async () => {
    try {
      ...
      await downloadTextFile(md, `${project.name}_${new Date().toISOString().slice(0, 10)}.md`, 'text/markdown')
  ...
  const handleExportTXT = async () => {
    try {
      ...
      await downloadTextFile(txt, `${project.name}_${new Date().toISOString().slice(0, 10)}.txt`)
  ...
  const handleReauthFolder = async () => {
    ...
    // 原生端（Android SAF）的系统授权无法再次弹窗恢复，只能重新选一次文件夹。
    if (isSafFolder(folderHandle)) { await handleBindFolder(); return }
    setFolderBusy(true)
    try {
  ...
  // 原生端（SAF）授权失效只能重新选目录，按钮文案随之区分
  const folderIsNative = folderHandle ? isSafFolder(folderHandle) : false
  ...
  return (
    <div className="space-y-4">
      ...
      <SectionCard
        ...
        badge={!isFolderBackupSupported() ? '当前环境不支持本地文件夹' : undefined}
      >
        {folderHandle ? (
          <div className="space-y-2">
            {folderNeedsAuth ? (
              <div className="flex items-center gap-2 text-sm text-amber-400 bg-amber-500/10 px-3 py-2 rounded-lg">
                ...
                <span className="flex-1 truncate">已绑定「{folderName}」，但写入授权已失效，需重新绑定才能自动写入</span>
          ...
          <ActionButton onClick={handleBindFolder} disabled={!isFolderBackupSupported() || folderBusy || status === 'loading'} variant="orange">
