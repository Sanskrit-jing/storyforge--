...
export default function LongformLayout({ children, title, section, mode = 'steps', module = 'info', hiddenModules, onSection, onMode, onModule, onHome, onNavigate }: {
  ...
  return <div className={`longform-app ${navigationOpen ? 'lf-navigation-open' : ''} ${stepsOpen ? 'lf-step-menu-open' : ''}`}>
    <header className="lf-top"><button className="lf-brand" aria-label="返回首页" onClick={onHome}><Flame/><span><strong>StoryForge</strong><small>故事熔炉</small></span></button><nav aria-label="产品导航"><button onClick={onHome}>首页</button><Link to="/?tab=worlds" onClick={event => { if (onNavigate) { event.preventDefault(); onNavigate('/?tab=worlds') } }}>世界引擎</Link><button aria-current="page" onClick={() => selectSection('library')}>长篇</button><Link to="/?tab=novel" onClick={event => { if (onNavigate) { event.preventDefault(); onNavigate('/?tab=novel') } }}>其他作品</Link></nav></header>
