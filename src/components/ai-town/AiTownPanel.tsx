...
export default function AiTownPanel(props: {
  ...
  view?: "play" | "relationships" | "history"
  ...
  const run = async (operation: () => Promise<void>) => {
    setLocalError('')
    try { await operation(); const id=useAiTownPlayerStore.getState().selectedSessionId; if(id!=null && id!==props.initialSessionId)props.onSessionSelected?.(id) }
  ...
  return <div className="flex min-h-[46rem] flex-col bg-bg-base xl:flex-row" data-testid="ai-town-player">
    <aside className="w-full shrink-0 border-b border-border bg-bg-surface p-4 xl:w-72 xl:border-b-0 xl:border-r">
      ...
      <p className="mb-4 text-xs leading-5 text-text-muted">正式游玩使用已发布版本；制作中的版本可从检查页试玩。小镇生活不会改变原作世界。</p>
      ...
      {!store.releases.length && !store.loading && <div className="rounded border border-dashed border-border p-4 text-center text-xs text-text-muted">尚无 AI 小镇发布。请在制作台完成制作与发布，或从检查页进入试玩。</div>}
      <div className="mb-2 mt-6 text-xs font-semibold">小镇存档</div>
      <div className="space-y-1">{store.sessions.map(session => <div key={session.id} className={`flex rounded ${session.id === selectedSession?.id ? 'bg-accent/10' : ''}`}><button className="min-w-0 flex-1 px-2 py-2 text-left text-xs" onClick={() => void run(async()=>{await store.select(session.id!)})}><strong className="block truncate">{session.title}</strong><span className="text-[9px] text-text-muted">{session.id === selectedSession?.id ? `事件 ${store.runtimeState.lastSequence}` : '可继续'}</span></button><button aria-label="删除小镇存档" onClick={() => void run(async()=>{if(await dialog.confirm({title:"删除小镇存档",message:"此操作不可撤销，确认删除？",tone:"danger"}))await store.remove(session.id!)})} className="px-2 text-text-muted hover:text-danger"><Trash2 className="h-3 w-3" /></button></div>)}</div>
    </aside>
    ...
    <main className="min-w-0 flex-1 p-4 sm:p-6">
      ...
      {selectedSession && town && <div className="mx-auto max-w-7xl space-y-4">
        ...
        <section hidden={!!props.view && props.view!=="play"} className="grid gap-4 xl:grid-cols-[minmax(0,1.4fr)_minmax(20rem,.8fr)]">
          <article className="rounded-lg border border-border bg-bg-surface p-4"><div className="mb-3 flex items-center justify-between"><h2 className="flex items-center gap-2 text-sm font-semibold"><MapPin className="h-4 w-4 text-accent" />小镇地图</h2><span className="text-[10px] text-text-muted">点击地点，拜访居民</span></div><div className="relative min-h-[25rem] overflow-hidden rounded-lg border border-border bg-gradient-to-br from-emerald-950/30 via-bg-base to-amber-950/20" data-testid="ai-town-map">
