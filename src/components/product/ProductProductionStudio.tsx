...
import {
  exportCommunityPrototypeDistributionBundleV2,
...
export default function ProductProductionStudio(props: {
  ...
  authorOptIn?: boolean
  view?: 'confirm' | 'production' | 'review' | 'release'
  ...
  const [aiTownSettings, setAiTownSettings] = useState<AiTownBriefSettingsV1>(DEFAULT_AI_TOWN_SETTINGS)
  ...
  const compileBrief = () => run(async () => {
    ...
    const next = await compileProductProductionBriefV3({
      ...
      aiTown: productType === 'ai-town' ? aiTownSettings : undefined,
  ...
  const authorize = () => run(async () => {
    if (props.authorOptIn === false) throw new Error('请先确认启用本作品的自动制作能力')
  ...
  const exportCommunityPrototype = () => run(async () => {
    if (!details?.production.id || !details.build) throw new Error('缺少可导出的 Build。')
    const bundle = await exportCommunityPrototypeDistributionBundleV2({
      scope: props.scope,
      productionId: details.production.id,
    })
    downloadJson(
      bundle,
      `storyforge-${details.production.productType}-community-prototype-b${details.build.buildNumber}.json`,
    )
    setMessage(`社区原型包已验签导出（${compactHash(bundle.bundleHash)}）；它不会取得商业 ProductRelease 身份。`)
  }, '导出社区原型包')
  ...
  const requestedCommercialImageReady = qualityProfile !== 'commercial-candidate'
    || !['avg', 'ttrpg', 'ai-town'].includes(productType) || (productType === 'ttrpg' ? ttrpgWizard.maximumGeneratedAssets === 0 : visualLevel === 'none')
  ...
  const requestedCommercialAudioReady = qualityProfile !== 'commercial-candidate'
    || (productType !== 'avg' && productType !== 'ai-town') || audioLevel === 'none' || capabilityReadiness.mediaRelayReady
    ...
    <main className="min-w-0 p-5 md:p-8">
      ...
      {!details ? <>
        ...
        <section className="grid gap-4 rounded border border-border bg-bg-elevated p-5 md:grid-cols-2">
          ...
          {productType === 'ai-town' && <fieldset className="grid gap-3 rounded border border-accent/30 bg-accent/5 p-4 md:col-span-2 md:grid-cols-3" data-testid="ai-town-production-settings">
            <legend className="px-2 text-xs font-semibold text-accent">后日谈小镇专属设置</legend>
            <label className="grid gap-1 text-[10px] text-text-muted">玩家身份<select value={aiTownSettings.playerRole} onChange={event => { setAiTownSettings(current => ({ ...current, playerRole: event.target.value as AiTownBriefSettingsV1['playerRole'] })); setDraft(null) }} className="rounded border border-border bg-bg-base p-2 text-xs text-text-primary"><option value="new-resident">新居民</option><option value="caretaker">共同体照料者</option></select></label>
            <label className="grid gap-1 text-[10px] text-text-muted">玩家称呼<input value={aiTownSettings.playerName} maxLength={100} onChange={event => { setAiTownSettings(current => ({ ...current, playerName: event.target.value })); setDraft(null) }} className="rounded border border-border bg-bg-base p-2 text-xs text-text-primary" /></label>
            <label className="grid gap-1 text-[10px] text-text-muted">小镇名称<input value={aiTownSettings.townTitle} maxLength={200} onChange={event => { setAiTownSettings(current => ({ ...current, townTitle: event.target.value })); setDraft(null) }} placeholder="留空则根据结局生成" className="rounded border border-border bg-bg-base p-2 text-xs text-text-primary" /></label>
            <label className="grid gap-1 text-[10px] text-text-muted md:col-span-2">玩家住处概念<input value={aiTownSettings.homeConcept} maxLength={500} onChange={event => { setAiTownSettings(current => ({ ...current, homeConcept: event.target.value })); setDraft(null) }} className="rounded border border-border bg-bg-base p-2 text-xs text-text-primary" /></label>
            <label className="grid gap-1 text-[10px] text-text-muted">距原作结局（天）<input type="number" min={1} max={100000} value={aiTownSettings.elapsedDays} onChange={event => { setAiTownSettings(current => ({ ...current, elapsedDays: Number(event.target.value) })); setDraft(null) }} className="rounded border border-border bg-bg-base p-2 text-xs text-text-primary" /></label>
            <label className="grid gap-1 text-[10px] text-text-muted">居民数量<input type="number" min={4} max={8} value={aiTownSettings.residentTarget} onChange={event => { setAiTownSettings(current => ({ ...current, residentTarget: Number(event.target.value) })); setDraft(null) }} className="rounded border border-border bg-bg-base p-2 text-xs text-text-primary" /></label>
            <label className="grid gap-1 text-[10px] text-text-muted">主要地点<input type="number" min={4} max={12} value={aiTownSettings.majorLocationTarget} onChange={event => { setAiTownSettings(current => ({ ...current, majorLocationTarget: Number(event.target.value) })); setDraft(null) }} className="rounded border border-border bg-bg-base p-2 text-xs text-text-primary" /></label>
            <label className="grid gap-1 text-[10px] text-text-muted">每日主动行动<input type="number" min={1} max={6} value={aiTownSettings.actionsPerDay} onChange={event => { setAiTownSettings(current => ({ ...current, actionsPerDay: Number(event.target.value) })); setDraft(null) }} className="rounded border border-border bg-bg-base p-2 text-xs text-text-primary" /></label>
            <label className="grid gap-1 text-[10px] text-text-muted">感情线<select value={aiTownSettings.romance} onChange={event => { setAiTownSettings(current => ({ ...current, romance: event.target.value as AiTownBriefSettingsV1['romance'] })); setDraft(null) }} className="rounded border border-border bg-bg-base p-2 text-xs text-text-primary"><option value="off">关闭</option><option value="canon-only">仅延续原作</option><option value="opt-in">玩家主动选择后开启</option></select></label>
            <label className="grid gap-1 text-[10px] text-text-muted">离线最多推进<input type="number" min={1} max={3} disabled={!aiTownSettings.offlineEnabled} value={aiTownSettings.offlineMaximumDays} onChange={event => { setAiTownSettings(current => ({ ...current, offlineMaximumDays: Number(event.target.value) })); setDraft(null) }} className="rounded border border-border bg-bg-base p-2 text-xs text-text-primary disabled:opacity-50" /></label>
            <label className="flex items-center gap-2 rounded border border-border bg-bg-base p-2 text-[10px] text-text-muted"><input type="checkbox" checked={aiTownSettings.offlineEnabled} onChange={event => { setAiTownSettings(current => ({ ...current, offlineEnabled: event.target.checked })); setDraft(null) }} />允许确定性离线演化</label>
            <label className="grid gap-1 text-[10px] text-text-muted">初始货币<input type="number" min={0} max={1000000000} value={aiTownSettings.startingMoney} onChange={event => { setAiTownSettings(current => ({ ...current, startingMoney: Number(event.target.value) })); setDraft(null) }} className="rounded border border-border bg-bg-base p-2 text-xs text-text-primary" /></label>
            <label className="grid gap-1 text-[10px] text-text-muted md:col-span-2">资源 key（1–3 个，逗号分隔）<input value={aiTownSettings.resourceKeys.join(', ')} onChange={event => { setAiTownSettings(current => ({ ...current, resourceKeys: event.target.value.split(/[,，]/).map(value => value.trim()).filter(Boolean).slice(0, 3) })); setDraft(null) }} className="rounded border border-border bg-bg-base p-2 text-xs text-text-primary" /></label>
            <label className="grid gap-1 text-[10px] text-text-muted md:col-span-3">共同建设目标<input value={aiTownSettings.sharedProjectConcept} maxLength={4000} onChange={event => { setAiTownSettings(current => ({ ...current, sharedProjectConcept: event.target.value })); setDraft(null) }} className="rounded border border-border bg-bg-base p-2 text-xs text-text-primary" /></label>
            <div className="flex flex-wrap gap-3 text-[10px] text-text-muted md:col-span-3"><span>冻结媒资意图：</span>{([['portraits', '居民肖像'], ['expressions', '表情差分'], ['locationCards', '地点卡'], ['ambientAudio', '环境音']] as const).map(([key, label]) => <label key={key} className="flex items-center gap-1"><input type="checkbox" checked={aiTownSettings[key]} onChange={event => { setAiTownSettings(current => ({ ...current, [key]: event.target.checked })); setDraft(null) }} />{label}</label>)}<span className="text-accent">语义地图固定启用</span></div>
            <p className="text-[9px] leading-5 text-text-muted md:col-span-3">重大死亡/永久离开/婚育/设施或世界规则变更固定需要作者确认；私密心智固定不可供玩家读取，这两项不能在界面放宽。</p>
          </fieldset>}
          <label className="grid gap-2 text-[10px] text-text-muted">视觉目标<select disabled={!['avg', 'ttrpg', 'ai-town'].includes(productType)} value={visualLevel} onChange={event => { setVisualLevel(event.target.value as 'none' | 'key-scenes'); setDraft(null) }} className="rounded border border-border bg-bg-base p-2 text-xs text-text-primary disabled:opacity-50"><option value="none">纯文字</option><option value="key-scenes">关键场景 + 角色素材</option></select><span>{productType === 'avg' || productType === 'ttrpg' || productType === 'ai-town' ? qualityProfile === 'prototype' ? '内容会自主拆分美术需求；原型档使用明确标记的安全占位素材。' : '内容拆分需求后，直接复用全局图片能力并行制作并接入玩家界面。' : '当前产品以专用文字舞台为主。'}</span></label>
          <label className="grid gap-2 text-[10px] text-text-muted">音频目标<select disabled={productType !== 'avg' && productType !== 'ai-town'} value={audioLevel} onChange={event => { setAudioLevel(event.target.value as 'none' | 'music-sfx'); setDraft(null) }} className="rounded border border-border bg-bg-base p-2 text-xs text-text-primary disabled:opacity-50"><option value="none">静音</option><option value="music-sfx">{productType === 'ai-town' ? '小镇环境音' : '主题音 + 关键音效'}</option></select><span>{audioLevel === 'music-sfx' ? qualityProfile === 'commercial-candidate' ? '音乐与音效需要独立音频能力，并和内容、美术并行，完成后自动接入 Cue。' : '音频与内容、美术并行；未绑定外部音频时使用明确标记的程序化 WAV。' : '不制作音频，产品仍可完整游玩。'}</span></label>
        ...
        {draft && <section className="mt-5 rounded border border-border bg-bg-elevated p-5"><div className="flex items-center justify-between"><h2 className="text-sm font-semibold">Brief v3 审查摘要</h2><code className="text-[9px] text-accent">{draft.intent.productType} / {draft.qualityProfile}</code></div><div className="mt-4 grid gap-3 text-[10px] md:grid-cols-3"><article className="rounded bg-bg-base p-3"><strong className="block text-xs">体验</strong><p className="mt-1 text-text-muted">{draft.intent.openingSituation}</p></article><article className="rounded bg-bg-base p-3"><strong className="block text-xs">规模</strong><p className="mt-1 text-text-muted">{draft.aiTown ? `${draft.aiTown.town.residentTarget} 位居民 · ${draft.aiTown.town.majorLocationTarget} 个主要地点` : `${draft.scale.targetPlayMinutes} 分钟 · ${draft.scale.targetEndingCount} 结局`}</p></article><article className="rounded bg-bg-base p-3"><strong className="block text-xs">完成合同</strong><p className="mt-1 text-text-muted">可玩预览 · 媒资覆盖 {Math.round(draft.completionContract.minimumMediaCoverage * 100)}%</p></article></div>{draft.aiTown && <div className="mt-3 grid gap-2 text-[10px] md:grid-cols-3" data-testid="ai-town-brief-summary"><span className="rounded bg-bg-base p-2">结局后 {draft.aiTown.continuity.elapsedDays} 天 · {draft.aiTown.clock.actionsPerDay} 次行动/日</span><span className="rounded bg-bg-base p-2">离线 {draft.aiTown.autonomy.offlineEnabled ? `最多 ${draft.aiTown.autonomy.offlineMaximumDays} 日` : '关闭'}</span><span className="rounded bg-bg-base p-2">重大变化人工确认 · 私密心智隔离</span></div>}{selectedSuggestion && <p className="mt-4 text-[10px] text-text-muted">起点冲突：{selectedSuggestion.openingConflict}</p>}<p className="mt-2 text-[10px] text-text-muted" data-testid="product-production-source-selection-summary">本次通过世界网关冻结 {draft.source.selection.resourceKeys.length} 项语义资源（{frozenSourceFacetSummary(draft, sourceOptions)}）；上层叙事、媒资和运行状态均由产品 Build 自己拥有。</p></section>}
        ...
        {progress && progress.tasks.length > 0 && <section hidden={Boolean(props.view && !['production'].includes(props.view))} aria-live="polite" className="mt-5 rounded border border-border bg-bg-elevated p-5">
          ...
          {taskEvidence && <section hidden={Boolean(props.view && !['production'].includes(props.view))} className="mt-4 rounded border border-border bg-bg-base p-4" aria-label="任务制作记录">
