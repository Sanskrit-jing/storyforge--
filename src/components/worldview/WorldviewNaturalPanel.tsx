  ...
  return (
    <div className="flex flex-col w-full h-full space-y-4">
      ...
      <div className="flex flex-1 flex-col gap-2 overflow-y-auto md:flex-row md:gap-0 md:overflow-hidden">
        {/* ── 侧栏：宽屏左侧纵向导航；窄屏顶部横向滚动标签条 ── */}
        <nav className="flex w-full shrink-0 gap-1 overflow-x-auto border-b border-border bg-bg-surface/50 pb-2 md:w-max md:min-w-32 md:max-w-44 md:flex-col md:gap-0 md:overflow-x-visible md:overflow-y-auto md:border-b-0 md:border-r md:pb-0">
          {[...FIELDS.map(f => ({ key: f.key, emoji: f.emoji, label: f.label })),
            ...
            return (
              <button
                ...
                className={`shrink-0 whitespace-nowrap rounded-lg px-3 py-2 text-sm transition-colors flex items-center gap-1 md:w-full md:rounded-none md:border-l-2 md:py-2.5 md:text-left ${
                  isActive
                    ? 'border-accent bg-accent/10 text-accent font-medium md:text-text-primary'
