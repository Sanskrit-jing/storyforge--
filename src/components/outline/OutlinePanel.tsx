...
import type { OutlineNode, Project, StoryStructure } from '../../lib/types'
...
import { computeChapterStepTarget, type ChapterReadingOrderEntry } from './chapter-drag'
...
export default function OutlinePanel({ project, onOpenChapter }: Props) {
  ...
  // ── 触摸端「上移/下移」（HTML5 拖拽需要鼠标，手指拖动会被浏览器判成滚动）──
  // 按阅读顺序（卷 → 故事块 → 章节）平移一位；跨卷/跨故事块时自动换父，效果与拖拽一致。
  const readingOrderChapters = useMemo(() => {
    const byOrder = (a: OutlineNode, b: OutlineNode) => a.order - b.order
    const ordered: ChapterReadingOrderEntry[] = []
    for (const volume of volumes) {
      const blocks = normalizedNodes
        .filter(node => node.parentId === volume.id && node.type === 'storyBlock')
        .sort(byOrder)
      const parents = blocks.length > 0 ? blocks.map(block => block.id!) : [volume.id!]
      for (const parentId of parents) {
        normalizedNodes
          .filter(node => node.parentId === parentId && node.type === 'chapter')
          .sort(byOrder)
          .forEach((chapter, siblingIndex) => ordered.push({ id: chapter.id!, parentId, siblingIndex }))
      }
    }
    return ordered
  }, [normalizedNodes, volumes])

  const handleMoveChapterStep = async (chapterId: number, delta: -1 | 1) => {
    const target = computeChapterStepTarget(readingOrderChapters, chapterId, delta)
    if (!target) return
    if (!canMoveChapterToParent(chapterId, target.targetParentId)) {
      toast.error('不能把章节移动到其它世界组的卷下。')
      return
    }
    await handleMoveChapter(chapterId, target.targetParentId, target.index)
  }

  const handleMoveVolumeStep = async (volumeId: number, delta: -1 | 1) => {
  ...
  const sidebarContent = (
    <OutlineVolumeSidebar
      ...
      onMoveVolumeStep={(volumeId, delta) => { void handleMoveVolumeStep(volumeId, delta) }}
    />
  )
    ...
    >
      <div className="p-4 space-y-4">
        ...
        <OutlineVolumeDetail
          ...
          onMoveChapter={handleMoveChapter}
          onMoveChapterStep={(chapterId, delta) => { void handleMoveChapterStep(chapterId, delta) }}
        />
      </div>
    </PanelLayout>
  )
