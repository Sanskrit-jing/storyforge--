...
import { Check, ChevronDown, ChevronUp, GripVertical, Layers, Loader2, Plus, Sparkles, X } from 'lucide-react'
...
interface OutlineVolumeSidebarProps {
  ...
  /** 触摸端替代拖拽的上移/下移（无鼠标的平板上也会显示） */
  onMoveVolumeStep: (volumeId: number, delta: -1 | 1) => void
}
        ...
        {volumes.map(volume => {
          ...
          return (
            ...
            >
              ...
              {/* 触摸端没有鼠标拖拽，用上移/下移按钮替代 */}
              <div className="hidden [@media(hover:none)]:flex flex-col shrink-0 pr-1">
                <button onClick={() => onMoveVolumeStep(volume.id!, -1)} title="上移一卷" className="p-0.5 text-text-muted active:text-accent">
                  <ChevronUp className="w-3 h-3" />
                </button>
                <button onClick={() => onMoveVolumeStep(volume.id!, 1)} title="下移一卷" className="p-0.5 text-text-muted active:text-accent">
