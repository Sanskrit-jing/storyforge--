...
export default function NodeModeWorkspace(props: {
  ...
  // 窄屏放不下「节点列表 + 画布 + 属性面板」三栏并排，改为分步切换
  const [narrowPane, setNarrowPane] = useState<'list' | 'canvas' | 'inspector'>('canvas')
  const hideOnNarrow = (pane: 'list' | 'canvas' | 'inspector') => (
  ...
  if (!draft) {
    return (
      <div className="flex h-full min-h-[60dvh] items-center justify-center bg-[radial-gradient(circle,var(--border-subtle)_1px,transparent_1px)] [background-size:28px_28px] md:min-h-[720px]">
  ...
  return (
    <div className="flex h-full min-h-0 flex-col overflow-y-auto bg-bg-base md:min-h-[720px] md:overflow-hidden">
      <header className="flex h-12 shrink-0 items-center gap-2 overflow-x-auto border-b border-border bg-bg-surface px-3 md:overflow-x-visible">
      ...
      {/* 窄屏切换：三栏无法并排，改为按需显示其中一栏 */}
      {isNarrow && (
        <div className="flex h-10 shrink-0 items-center gap-1 border-b border-border bg-bg-surface px-2">
          {([['list', '节点列表'], ['canvas', '节点画布'], ['inspector', '属性面板']] as const).map(([pane, label]) => (
            <button
              key={pane}
              type="button"
              onClick={() => setNarrowPane(pane)}
              aria-pressed={narrowPane === pane}
              className={`flex-1 rounded px-2 py-1.5 text-xs ${narrowPane === pane ? 'bg-accent/15 text-accent' : 'text-text-secondary hover:bg-bg-hover'}`}
            >
              {label}
            </button>
          ))}
      ...
      <div className="flex min-h-0 flex-1 flex-col md:flex-row">
        <aside className={`${hideOnNarrow('list')} min-h-0 w-full flex-1 shrink-0 overflow-y-auto border-b border-border bg-bg-surface p-3 md:w-52 md:flex-none md:border-b-0 md:border-r`}>
        ...
        <div className={`${hideOnNarrow('inspector')} min-h-0 w-full flex-1 md:w-80 md:flex-none`}>
          <NodeInspector
            projectId={projectId}
      ...
      <section className="relative shrink-0 border-t border-border bg-bg-surface">
        ...
        {showRunDetails && (
          <div className="grid max-h-64 grid-cols-1 gap-0 overflow-y-auto border-t border-border md:grid-cols-2">
