...
export default function WorldResourceBrowser({projectId,worldId,onEdit,onVersions}:{projectId:number;worldId:number;onEdit:()=>void;onVersions:()=>void}){
 ...
 useEffect(()=>{let active=true;void resolveScopeLike(projectId).then(scope=>listWorldReleases(scope)).then(rows=>{if(active){setReleases(rows);setReleaseId(rows[0]?.id??null)}}).catch(c=>{if(active)setError(String(c))});return()=>{active=false}},[projectId])
