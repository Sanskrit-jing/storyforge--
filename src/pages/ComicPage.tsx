...
import { COMIC_PAGES, COMIC_GROUPS, comicPageDescription } from '../components/comic/navigation'
...
export default function ComicPage(){
 ...
 <section ref={main} className="lf-main"><header className="lf-heading"><small>小说转漫画 › {pages.find(([id])=>id===current)?.[1]}</small><h2>{pages.find(([id])=>id===current)?.[1]}</h2><div className="lf-mobile-controls"><button onClick={()=>setMenu(!menu)}>漫画导航</button></div></header><div className="lf-body"><div className="lf-content">{(COMIC_GROUPS.find(group=>group.pages.includes(current))?.pages.length??0)>1&&<nav className="cp-subnav" aria-label="漫画内容分类">{COMIC_GROUPS.find(group=>group.pages.includes(current))?.pages.map(id=><button key={id} aria-current={id===current?'page':undefined} onClick={()=>void go(path(id))}>{pages.find(([key])=>key===id)?.[1]}</button>)}</nav>}
