...
export default function HomePage() {
  ...
  const handleRestoreFromFolder = async () => {
    if (!isFolderBackupSupported()) { setRestoreMsg('当前环境不支持本地文件夹，请用 Chrome / Edge 或安装 Android 版') ; return }
  ...
  return (
    <div className="min-h-screen bg-bg-base" onClick={() => setDeleteConfirm(null)}>
      ...
      {/* ── 顶栏 ──────────────────────────────────────── */}
      <header className="border-b border-border px-4 py-4 flex items-center sticky top-0 bg-bg-base/90 backdrop-blur-sm z-20 md:px-8">
      ...
      <main className="max-w-3xl mx-auto px-4 py-8 md:px-8 md:py-12">
        ...
        <div className="mb-8">
          ...
          <h1
            ...
            style={{ fontFamily: 'var(--font-serif)', fontSize: 'clamp(28px, 7.5vw, 40px)', fontWeight: 400, letterSpacing: -0.6, lineHeight: 1.15 }}
          >
            {projects.length > 0
        ...
        <div>
          {loading ? (
            ...
            <>
              {projects.map((project) => {
                ...
                return (
                  <div
                    ...
                    className="flex items-center gap-3 py-5 border-b border-border cursor-pointer group transition-all hover:rounded-lg hover:border-transparent hover:bg-bg-hover md:gap-5 md:hover:px-2"
                  ...
                  >
                    ...
                    <div
                      ...
                      style={{ fontFamily: 'var(--font-serif)', fontSize: 'clamp(26px, 8vw, 34px)', fontWeight: 400, width: 'clamp(30px, 11vw, 44px)', textAlign: 'center', lineHeight: 1 }}
                    >
                      {glyph}
                    </div>
              ...
              {/* 新建行 */}
              <div
                className="flex items-center gap-3 py-4 text-text-muted cursor-pointer group hover:text-accent transition-colors md:gap-5"
                onClick={() => setShowCreate(true)}
              >
