...
export default function WorkspacePage() {
  ...
  // 窄屏（< md）下侧栏无法常驻，收成抽屉由顶栏菜单键唤出；宽屏该状态不生效（见下方 md: 覆盖）
  ...
  return (
    <div className="h-dvh safe-area-pad bg-bg-base flex overflow-hidden">
      ...
      >
        <Sidebar
          ...
          onSelect={(m) => {
            ...
            if (m !== 'editor') setEditorNodeId(null)
          }}
          onBack={() => navigate('/')}
      ...
      >
        <div className="flex h-11 shrink-0 items-center justify-between gap-2 border-b border-border/70 bg-bg-surface/70 px-3 md:gap-3 md:px-4">
          <div className="flex min-w-0 items-center gap-2">
            <button
              onClick={() => setMobileNavOpen(true)}
              title="打开导航"
              aria-label="打开导航"
              className="shrink-0 rounded p-1.5 text-text-muted transition-colors hover:bg-bg-hover hover:text-text-primary md:hidden"
            >
        ...
        <div className={`min-h-0 flex-1 overflow-y-auto ${isImmersiveModule ? '' : 'p-4 md:p-6'}`}>
