...
## v3.9.2（补录） — 2026-09-21 · 显式保存 + 测试连接 + 依赖安全升级

> 以下条目随 v3.9.2 Release 重发版一并发布，此处补录存档。

- 修复「AI 从内容拆分词条」等非流式调用遇到中转网关 500「操作失败」必然失败的问题：非流式 `chat()` 此前没有任何自动重试（流式 `streamChat` 有 429/503 重试），现统一共享可重试状态码集合（429/500/502/503/504，中转网关常把上游偶发波动包装成 500），最多重试 2 次（2s/4s 退避）；4xx 参数/鉴权类确定性错误仍立即失败不重试。
- 修复手机竖屏/HD 版 AI 生成结果底部操作栏按钮组折成两行的问题：按用户要求改为强制单行横排——按钮组容器 `flex-nowrap` 不再换行，窄屏自动缩小按钮（内边距/字号/间距收紧，`sm:` 断点恢复常规尺寸），字数/Token 信息放不下时仍折到按钮上方。
- 修复「AI 从内容拆分词条」拆分结束后弹窗无任何结果反馈的问题：其一，AI 调用/解析异常此前被 `try/finally` 静默吞掉，现补 `catch` 分支，错误以 toast + 弹窗内持久错误条展示；其二，候选结果渲染在弹窗滚动区底部、窄屏一屏放不下且不自动滚动，易被误判为「没有结果」，现拆分完成后自动平滑滚动到候选区并显示「拆分完成 · 共 N 条候选」标题；其三，成功但零候选时在弹窗内持久提示来源内容缺少可独立成条的设定，不再只靠会自动消失的 toast。
- 移除「Edge 直连微软 TTS」功能（整条链路，实测不可用）：微软对非 Edge UA 一律 403 拒绝、版本/时钟门禁苛刻且随时变动，真实环境中基本无法稳定使用——按用户反馈整体移除。设置页删除「方式一 · Edge 直连」整块配置区（含 UA 判定与启用开关），引擎链路收窄为「自部署 Worker > 浏览器内置 Web Speech API」两档，删除 edge-direct 引擎实现与对应回归测试，朗读失败指引同步更新；非 Edge 环境获得 Neural 人声只保留自部署 Worker 一条途径。历史存储中的已废弃 preferEdgeDirect 字段读取时自动忽略，不影响旧数据。
- 设置页「语义检索(embedding)」与「朗读人声」两张配置卡由「输入即静默保存」改为「草稿 + 显式保存」模式并补齐保存反馈：此前两张卡均为输入即写持久层、无保存按钮（朗读人声卡底部写着「保存后重新进入章节编辑器生效」却没有任何保存动作），用户无从感知配置是否已被记住。现输入/预设/开关只改本地草稿，无更改时「保存」按钮禁用，有更改时显示「有未保存更改」提示，点「保存」才写入持久层并提示「已保存」；朗读人声「清空」仍即时生效（恢复浏览器内置语音）。
- 修复依赖审计闸门被上游安全公告阻塞的问题：@tiptap 全家桶 3.28.0 → 3.31.3（修复 GHSA-cp6q-959q-f8rh 原型污染与 GHSA-j95f-988m-3j2f ReDoS），生产依赖审计恢复 0 漏洞，CI 依赖闸门恢复全绿。
- 语义检索配置卡新增「测试连接」：发一条真实嵌入请求验证 Base URL/嵌入模型/API Key 与网络连通性，成功显示返回向量维度并提醒未保存的更改、失败给出排查方向（地址/模型/Key/网络，本地 Ollama 需已启动），不写入任何检索索引；「建立检索索引」仍使用已保存的配置。
- 修复手机竖屏/HD 版全屏查看层（AI 生成结果全屏、全屏文本编辑及各类弹窗）顶部被手机状态栏/手势条遮挡的问题：统一按安全区（safe-area inset）避让，PC 版间距不变。
- 新增共享组件 FullScreenTextarea：全项目多行输入框在手机/HD 上去掉原生拉伸手柄，右下角改为「全屏查看并编辑」按钮，全屏层内编辑完成后经 onBlur 正常落库；PC（≥1280px）保留原生气泡手柄且不显示按钮。
- 新增共享组件 FieldGenerationBar：世界观/故事核心等字段级 AI 操作行在手机/HD 下改为两行布局（扩写/重写/润色 + 生成按钮一行、补充说明整行一行并支持全屏输入），不再挤压遮挡；PC 保持单行。
- 修复手机竖屏/HD 版角色关系、角色生成、消耗统计等工具栏文字被压成竖排、按钮溢出或右侧大片留白的问题：按钮统一 nowrap + 自动换行，角色生成工具栏窄屏整行重排，消耗统计明细表支持容器内横向滚动。
- 修复手机竖屏/HD 版 AI 生成结果底部操作栏（字数/Token/全屏/重试/好示例/反例/采纳）被压缩、采纳按钮被裁切的问题：操作栏自动换行，按钮组不收缩。
- 修复角色详情一句话简介在窄屏被压成窄列的问题：简介与「AI 补全设定」操作在窄屏下通栏显示、操作按钮仅显图标释放宽度。
- 「AI 从内容拆分词条」在来源全貌为空时改为明确提示粘贴内容（功能本身正常，并非拆分失败或被遮挡），拆分弹窗文本域支持全屏编辑。
- 修复手机竖屏/HD 版正文选区浮动工具条（润色/扩写/缩写/改写/查漏）图标文字被压成竖排的问题：按钮统一横排并按宽度自动换行，浮条按视口左右夹取且选区上方空间不足时自动弹到下方；窄屏下新增「本次写作要求」两阶段流程（可手填要求或点「AI 填写要求」，再按要求生成/直接生成），PC（≥1280px）保持原一键直出。
- 修复手机竖屏/HD 版故事年表、物品栏、状态表、修炼进度、事实库、世界宪法、伏笔、场景考证、修炼体系、世界观等面板头部说明文字右侧大片空白的问题：头部窄屏统一改为整行重排（标题/描述占满一行、操作按钮下一行靠右），PC 保持原左右布局。
- 修复手机竖屏/HD 版大纲页左上「大纲」折叠标签悬浮突兀的问题：窄屏改为底部居中胶囊按钮，PC 保持原左上标签位置。
- 修复手机竖屏/HD 版大纲章节卡摘要被拖拽柄、序号和常驻操作按钮压成窄列、右侧空白的问题：章节卡窄屏两行排布（首行序号标题+操作、次行摘要整行），PC 保持单行。
- 章节创作页手机竖屏/HD 下新增「本章辅助信息」整体折叠条（本章目标、情感节拍、AI 高级选项、上下文预算、章节摘要/对账等），默认收起，正文编辑区常驻首屏；按需展开或单独弹出的面板不受影响，PC 无此折叠条。
- 修复手机竖屏/HD 版章节 AI 工具栏（生成正文/续写/扩写/润色等 10 个动作）大小不一、未对齐的问题：窄屏统一为 3 列（HD 4 列）等宽网格，自定义指令输入整行；PC 恢复原自然宽度换行。
- 章节正文新增语音朗读（听书）：章节标题栏新增「朗读」按钮，基于浏览器/系统原生语音（Web Speech API，无需 API Key、离线可用），支持播放/暂停、上一段/下一段、停止、五档语速（0.7–1.5x）与中文语音选择；按正文段落逐段朗读，当前段落在编辑器中高亮并自动滚动；超长段落自动按句切分以规避浏览器长文本截断；切换章节、进入对照润色或离开页面时自动停止；语速/语音偏好跨章持久化；不支持语音合成或系统无可用语音的环境自动隐藏入口或给出明确提示。
- 语音朗读自动选择更自然的人声：默认语音不再取列表中第一个普通话语音（Windows 下常落到老引擎 SAPI/OneCore 语音，听感机械），而是同语言内优先神经网络语音（Edge 的「… Online (Natural)」等 Natural/Neural 人声，其次 Online 新引擎）；语音下拉按自然度排序置顶；环境缺少自然语音时在控制条提示改用 Edge 浏览器或在系统设置安装「自然」中文语音。
- 朗读控制条支持自由移动、最小化与隐藏：按住控制条空白处可拖到屏幕任意位置（位置记忆，刷新后保留，自动钳制在可视区内）；「—」收起为悬浮小球（播放/暂停、进度、展开、停止），不再遮挡正文；「眼睛」图标完全隐藏控制条且朗读继续，顶部「朗读」按钮朗读中变为控制条显示/隐藏开关，停止统一走控制条「×」。
- 语音选择器增强发现性：语音下拉按自然度分组（「自然语音 · 真人感」置顶，其余归「其他语音」），新增「试听」按钮以固定 1 倍速播放样例句快速试音（朗读进行中需先暂停再试听）；试听与分组复用同一自然度判定，与自动选择行为一致。
- 朗读新增远程 TTS 引擎（Edge Neural 人声）：设置页新增「朗读人声（远程 TTS）」配置卡，可接入自部署的 edgetts-cloudflare-workers（免费 Cloudflare Workers 代理微软在线 TTS），为手机版 Edge、APK/HAP 打包后 WebView 等缺少 speechSynthesis/Neural 人声的环境提供与 Edge 桌面版同款的晓晓/云希等 Neural 人声（内置 15+ 中文声库清单、流式 mp3 经 Audio 播放、支持测试连接）；未配置时行为不变（浏览器 Web Speech API），配置后正文文本会发送到所部署的 Worker 与微软在线 TTS，服务不可达/Key 失效/服务端错误会中止朗读并给出明确提示，重新进入章节编辑器后生效。
- 朗读新增 Edge 直连引擎（零部署 · 实验性）：与 Edge「大声朗读」同源的微软私有消费端接口直连（WebSocket + Sec-MS-GEC 时钟签名），无需部署 Worker 或 API Key 即可使用晓晓/云希等 Neural 人声；设置页新增「Edge 直连（零部署 · 实验性）」开关（默认开启，UA 不符时自动跳过），引擎优先级为 Worker > Edge 直连 > 浏览器内置。可用条件：用 Edge 浏览器打开本页，或打包壳（APK/HAP）将 WebView 的 User-Agent 自定义为 Edge 移动版格式（`setCustomUserAgent` 设置含 `EdgA/版本号` 的完整 UA，版本须较新）——手机没有 Edge 浏览器也能用 Neural 人声；设备时钟需基本准确（签名按 5 分钟取整）。合成失败或服务端拒签会中止朗读并提示，可改配 Worker 或关闭开关回落浏览器内置人声；超长段落自动按 4096 字节分块、逐块等待服务端确认后再发下一块，服务端中途断连会被正确识别为不可用而非悬挂。
- 朗读设置页「朗读人声」拆分为「方式一 · Edge 直连」「方式二 · 自部署 Worker」两个带徽章与适用环境说明的分区：方式一明确仅 Edge 浏览器/自定义 UA 打包壳可用（微软实测对非 Edge UA 一律 403 拒绝、浏览器安全限制不允许网页伪造 UA，非 Edge 环境给出 Edge 浏览器/打包壳自定义 UA/改用方式二三条出路）；方式二给出 Cloudflare Workers 注册、一键部署、粘贴地址测试的三步指引，说明其为非 Edge 环境使用 Edge Neural 人声的唯一途径；朗读失败弹窗按当前引擎来源（Worker 连接失败/Edge 直连被拒/系统无语音）给出针对性排查指引。
- 修复 Edge 直连在多数真实 Edge 版本下必然失败的问题：Sec-MS-GEC-Version 此前固定为 143.0.3650.75，而微软实测校验该版本与运行环境 User-Agent 中 Edge 版本的一致性，错配一律 403——现改为从 UA 动态提取真实版本号（Edg/ EdgA/ EdgiOS/ 均覆盖）；同步实测确认另两条门禁并写入失败提示：设备时钟偏差 10 分钟即被拒、版本过老（如 131）即使报文自洽也被拒需升级 Edge。
- 修复 Edge 直连在非安全源（如 `http://局域网IP` 访问本页）必然失败的问题：此类页面浏览器禁用 `crypto.subtle`（WebCrypto），Sec-MS-GEC 时钟签名在 WebSocket 创建前即抛错，表象与握手失败完全一致——现签名计算在 `crypto.subtle` 不可用时回退内置纯 JS SHA-256 实现（FIPS 180-4，与 WebCrypto 输出一致并有独立参考向量回归钉住），非安全源页面完成签名后即可正常握手；安全源行为不变。

## v3.9.2 — 2026-09-20 · 移动端显示修复

- 修复手机竖屏下场景细纲行内「字数」输入框被挤出卡片右缘被裁切的问题：场景标题输入框允许收缩，节奏/字数/删除控件不再被压缩。
- 修复手机横屏时内容区上下出现空白、未铺满全屏的问题：viewport 启用 `viewport-fit=cover`，页面按安全区（safe-area）自适应系统栏，不再由原生视图内边距留白。
- 修复手机竖屏/HD 版节点模式头部「待保存到本地」状态文字被压成一字一行竖排且被遮挡的问题：头部改用响应式布局，窄屏下状态文字移至三栏切换行，按钮组与缩放控件加固（`shrink-0`）。
- 修复手机竖屏/HD 版资料检索来源元信息（字数·tokens·更新时间）被压成竖排、默认权重控件溢出面板右缘的问题：元信息单行截断显示，窄屏下权重控件移入展开区首行，HD/PC 仍保留在摘要行尾。
- 修复手机竖屏/HD 版全局故事线卡片「支线」徽标竖排、删除按钮超出卡片右缘的问题：名称输入框 `min-w-0` 收缩让位，徽标 `whitespace-nowrap`，删除按钮 `shrink-0` 收回卡片内。

## v3.9.1 — 2026-08-04 · 分步骤模式最终稳定版

> 这是 StoryForge 以传统分步骤模式为主的最后一个稳定发布包。后续开发将转向 StoryForge 社区新形态：平台综合首页、世界引擎、节点创作、跑团、角色聊天和文字游戏等并列功能入口。新形态不会改变本版本已有项目和手稿的数据安全边界。

- 继续提供稳定的分步骤世界观、角色、大纲和正文创作流程，适合作为现有项目的最后一个传统工作台版本。
- 保留本地优先数据、JSON 导入导出和源码版 npm 启动方式；本次发布不提供 `.bat`、`.exe` 或 Windows Portable 启动器。
- 未来的社区版本会以世界引擎作为可复用基座，让世界设定可以被小说创作、节点编排、跑团、角色聊天和文字游戏等功能引用；这些新入口不属于本发布包。
